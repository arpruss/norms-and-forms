...
The form of argument for Aristotelianism is similar to David Lewis's argument for extreme modal realism 
\parencite{LewisPlurality1986}: the variety of problems solved by the theory is evidence for the theory.
...
that such nose-thumbing is an epistemic virtue. Likewise, to use \citeauthor{McInernyET}'s (\citeyear{McInernyET}, p.~2) vivid example, eating thumbtacks does not contribute to flourishing no matter how much one wants 
to eat them or how much one enjoys this due to a rewiring of one's pleasure system. A non-realist reader will say I am just thumping the table, of course. 
...
I have marked sections and footnotes of a technical nature that can be safely skipped by a reader with an asterisk.
...
like oak trees and human beings to be paradigm cases of substances. In fact, not only are substances not limited to 
microphysical entities, but there is some reason to doubt that that microphysical entities are substances at all. For 
one can have a superposition of states with different numbers of particles of a given type, whereas there should be a well-defined 
...
thinkers to suppose a single universal substance---as in quantum holism \parencite{IsmaelSchafferQHolism}---and that if one tries to avoid
that conclusion, doing so will offer some way of restoring particle-based substances. (I am grateful to Robert Verrill 
for
this objection.)}
...
albeit in relevant respects they are all exactly alike in substances of the same kind. For two things could in
...
individual forms, as we shall see, and on the whole I will argue in Section~\ref{ch:details}.\ref{sec:indform} that there is good
...
optimistic thought that natural states occur ``for the most part'' \parencite[\textit{Parts of Animals}, III.2]{Aristotle}, which is quite useful for figuring out
what is in fact natural, since the frequency of the occurrence of a state is evidence of its naturalness.

There is, however, a tension in Aristotle's own thought between the above optimism and his pessimistic thought that
few are virtuous. Exploring the tension will help us understand Aristotelian optimism more clearly as it faces the
...
states.\footnote{For further discussion of the harmony between substances, see Chapter~\ref{ch:God}.} But Aristotle nonetheless
...
in his \textit{Monadology} estimated that three quarters of our activity is at an animal level \parencite[Sect.~28]{RescherMonadology}. Stalin was an utter moral failure, but
...
It is a good thing to eat your breakfast, but you don't fancy it's very good of you to do it. The thing is good---not you.'' \parencite[p.~32]{MacDonaldPC}
\end{quote}
...
take this further and \textit{define} the human being as a rational animal, and indeed this is common from the 
medieval Aristotelian tradition\footnote{For instance, Aquinas grants that ``seeing that man adds
rational to animal, it follows that a rational animal is a man''~\parencite[Q9 A1 Rp.~1]{AquinasPower3}.} to  
recent times. It follows that if rational dolphins or octopuses evolved, they would also be human in the philosophical sense, having the 
same nature as we do. However, while Aristotle himself certainly held that we were rational and we were animals, he did 
apparently does not \textit{define} us by our rational animality~\parencite{Kietzmann2019}.
...
mode of definition\footnote{However, see \citebyname{KoonsPorphyrean} for a promising contemporary account of the Porphyrian tree.}, but it would be 
...
However, this argument is much too quick. After all, the variety-of-normalcy problem may well arise even within the 
biological species \textit{Homo sapiens}, though particular examples are apt to be controversial. 
The majority of the human adult population worldwide, especially outside of Europe, is lactose intolerant.
At the same time, sometimes lactose intolerance is described as an ``impaired ability to digest lactose''~\parencite{CDHF},
...
Yet this alleged impairment is found in the majority of the human adult population worldwide, with prevalence
varying regionally from below 5\% to nearly 100\%~\parencite{Sahi1994}.
We could say that speaking of lactose intolerance as an impairment is just a mistake. But there is another possibility: it may be that lactose intolerance is normal for some members of our species and abnormal for others, and the description 
of it as an impairment is correct when restricted to persons of certain ancestries but not others.
In that case, assuming that all \textit{Homo sapiens} do share the same nature, what makes it abnormal for an adult
of one ancestry to be lactose intolerant but normal for an adult of another 
...
members of Homo sapiens. For instance, according to one study, rescuers of Jews from the Holocaust were more likely than non-rescuers (39\% vs.\ 15\%)
to emphasize that ``ethical values were to be applied universally, that they extended to all human beings''~\parencite[p.~165]{Oliner1988}. The intuition of many rescuers, as well as of typical anti-racists and 
...
criteria for developed personhood of philosophers like \citeauthor{WarrenAbortion} (\citeyear{WarrenAbortion}), such as language or generalized problem-solving skills? 
...
The remaining subquestion deals with immature members of Homo sapiens, such as zygotes, embryos, fetuses and infants, prior to the
...
interpretation of maturation processes. Death seems to be a catastrophic event for a substance, and in the ordinary
growth of from zygote to embryo to fetus to infant to child to adult 
there does not appear to be such a catastrophe. This suggests that these members of our biological species are also human in the metaphysical sense, but more needs to be said~\parencite{PrussAbortion, ColgroveZygotes}.
...
I could equally ask you about Saturn, and Jupiter, and Mars ... 
...
...
\parencite[pp.~97--98,282--283]{MersenneImpiety}\footnote{The moon-earth distance is approximately correct. The earth-sun distances are an order of magnitude off.}
\end{quote}
These ``Mersenne questions'' go on and on, with a mind-numbing number of examples. And Mersenne has one answer to all
these questions, posed in a rhetorical question: ``Was it not God?''~\parencite[p.~283]{MersenneImpiety}.

The argument sounds similar to fine-tuning arguments for theism which became popular in the late 20th century. These
arguments, too, list a variety of physical parameters and offer God as an explanation of them 
all.\footnote{For one of many defenses, see \citebyname{LewisBarnes}, and for one of 
many criticisms, see \citebyname{MMV2001}.}
Now, a crucial
...
as we know it, or for some other valuable trait of the universe, like its amenability to scientific investigation~\parencite{CollinsDiscoverability}.
...
explanation of  the evolution of the size of the moon's orbit~\parencite{Canup2014}, and hence of the current distance between the earth and the moon. 
...
But Mersenne's argument has a fatal flaw. Even if we grant that it is very unlikely that a future science will predict
...
The appearance of arbitrariness is evidence of contingency, and contingency calls out for explanation.\footnote{In \citebyname{PrussPSR}, I have
...
point is very similar to an argument in \citebynamewithpages{Hume1779}{Part~IX}.} Thus the appearance of arbitrariness of a parameter is merely
