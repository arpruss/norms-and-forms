...
There are competitive games, such as the Prisoner's Dilemma or the Tragedy of the Commons~\parencite{Hardin}, where it is difficult 
...
the terminology of David Lewis's pioneering study of coordination games~\parencite{LewisConvention}, each of these is also a coordination 
...
behaviors, in a kind of virtuous significance-conferring circle \parencitecf{SellarsEPM}. But if so, then we can just count the whole circle 
...
predator and does not squeak when not presented with a predator. Knowing this prior is wide-spread in the community, Alice can squeak in order to 
...
Wodehousian situation, one may own an ugly heirloom that one cannot give away because 
...
The economist Thomas Schelling~\parencite{Schelling} performed a number of experiments on coordination, in the context of 
...
what kind of a normative \textit{effect} it produces. A number of philosophers have thought that assertion does create
a \textit{prima facie} obligation to correct one's statement if one's statement turns out to be false~\parencite[p.~165]{Dummett1991}. If this is the
...
But an Aristotelian can hypothesize the following story, building on the concept of a normative powers~\parencite{Raz1972,
Lewinsohn2024, DurhamDissertation}.
Say that a normative effect is one that affects normative truths.
Then:
...
fictionalization or stage-acting might be accounted for as simulations or
pretences to other kinds of speech acts.
...
\section{Content and indeterminacy of reference}\label{sec:indet-ref}
Wittgenstein, \citeasnoun{Kripkenstein}, \citeasnoun{QuineWO} and \citeasnoun{PutnamRTH} have problematized reference and content in light of the fact that different
...
the integers---would be underdetermined even by a countably infinite number of statements about them. 
...
based on insistence on classical logic~\parenciteeg{SorensenVagueness}, push in favor of determinacy, but alas also in cases where we expected indeterminacy.
...
\citeasnounwithpages{Faroldi}{137}, you might be obligated to drive to the hospital, without being obligated to either drive to the hospital or 
...
explanations are always hyperintensional.\footnote{Cf.\ \citeasnounwithpages{Faroldi}{139}. Faroldi restricts this to non-causal explanations, but
...
comes in two varieties. One variety is famously pointed out by \citeasnoun{BenacerrafIdentification}: there are many sets of abstract objects
...
natural number structures we mean in a way sufficient to determine the truth or falsity of all arithmetical sentences.
...
but as was argued in earlier chapters (especially Chapter~\ref{ch:ethics}), the norms in human nature can indeed be very complex. 
...
it nor a reason not to make it. But now recall the argument that moral evaluation 
cannot be non-epistemically vague (Section~\ref{ch:ethics}.\ref{sec:two-step-vagueness}. If moral evaluation were more than epistemically vague, and yet we had classical logic, then the right account 
...
\subsubsection{Logic}
A standard argument for full-blown epistemicism is the Sorites series~\parenciteeg{SorensenVagueness}.
...
consider the case of a racist who writes a series of factually correct and interesting articles, each one 
about a different highly immoral member of the very same minority group. Each
...
\subsubsection{A sharp world and a somewhat fuzzy language}\label{sec:somewhat-fuzzy}
...
about the world~\parencite{MerricksVV}.
...
can do this by fully precisely grounding our semantic norms---for, as we saw in Section~\ref{sec:somewhat-fuzzy}, it is possible to have fully precise
...
three are that you (respectively) believe~\parencite{Bach2008}, justifiably believe~\parencite{Kvanvig2009}, or know~\parencite{WilliamsonKn} $p$.\footnote{There may also be some edge cases where none 
...
not clear that in ordinary assertions you need to have any beliefs about such complex conditionals~\parencite{PrussLying}. These complications
...
But that's too quick. We learn from \citeasnoun{BenacerrafIdentification} that the concept of \textit{the} natural numbers is dubious. Set theorists
...
go for the sequence of sets $\varnothing, \{\varnothing\}, \{\{\varnothing\}\},...$. Or, following Frege, we might 
...
G\"odel sentence~\parencite[44]{Batzoglou} for these axioms, chosen so that $G$ is a $\Sigma_1$ sentence, i.e., has only one unbounded quantifier, 
which quantifier is existential. Thus, $G$ is of the form $\exists n F(n)$, where all the quantifiers in $F(n)$ are 
...
that a certain formula $\phi_M$ of first-order logic is valid, i.e., is provable from a tautology~\parencitecf{Turing}. Putting all this together,
...
something like greater ``naturalness'' in the Lewisian sense~\parencite{LewisNewWork}. Or we might accept causal finitism~\parencite{KoonsKalaam,PrussInfinity},
which builds the concept of the finite---and hence of a natural number---into the metaphysics underlying causation. 
Or we might simply suppose a ``magical'' theory of reference on which there is a special relation between the human 
mind and a certain collection of mathematical entities~\parencitecf{PutnamRTH}. And some think that moving to second-order logic solves the
difficulty.
...
semantics~\parencite[ch.~4]{Enderton}. This seems to solve all our problems at little cost beyond allowing second-order
...
allow for non-standard semantics for second-order quantifiers, the Henkin semantics, where we do not 
interpret second-order quantifiers in terms of subsets, then we lose 
the unique characterization of the natural numbers, and our old problem is unsolved.\footnote{$^*$There is
an internal categoricity result in Henkin models which generalizes the standard-model categoricity
result by showing that the isomorphism of arithmetic across all models within a fixed Henkin 
model~\parencite{InternalCat}, but that does not seem to solve 
the problem of picking out the true arithmetic.}
