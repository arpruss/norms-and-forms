...
\subsection{The rule of preferential treatment}\label{sec:preferential}
...
the degrees of relation are relevant to the moral evaluation. (For further discussion of utilitarianism, see Sections~\ref{sec:act-util}-\ref{sec:rule-util}.)

Indeed, whatever the function is, it is likely to look arbitrary. Fix the degrees of relationship to be those of one's parent and of a total stranger,
...
utilitarian proposal, we do not have any at all plausible simple proposal for $f$. And when we realize that there is
a large number of other factors besides degrees of relationship and degrees of benefit---for instance, surely children owe
more to those who have put more effort and genuine love into parenting\footnote{For a fictional exploration of some of these
complexities, see \posscite{DaviesST} \textit{Salterton Trilogy}.}---the idea of a simple proposal for a determining rule becomes
quite untenable.
...
that cruelty is forbidden rather than commanded provides an unacceptable answer to the Euthyphro problem of explaining
why God prohibits what he prohibits.
...
\subsection{Risk and uncertainty}\label{sec:risk}
...
that there is a broad range of reasonable attitudes.
...
do.  And of course things are more complicated than described above. Plausibly, the rational evaluation function 
will depend not just on the probabilities involved but also on the values of the potential gains and losses, and
perhaps even on the \textit{types} of values (lives, money, etc.) that are involved.\footnote{$^*$There are technically two ways of taking risk
into account in decision theory. The classical approach is to apply a non-affine monotonic non-decreasing 
transformation function $f$ to the original utility or value $U$ assignment on the epistemic probability space 
and then simply maximizing expected rescaled 
utility $E[f(U)]$~\parencite[Section~II]{WilkinsonRisk}.
On the other hand, \citebyname{BuchakRisk} offers a novel way of calculating expected utilities without rescaling them.
In the special case where the utility function $U$ is non-negative, this consists in applying a monotonic
transformation $g$ and replacing $E[U]$ with $\int_0^\infty g(P(U>\lambda)) \, d\lambda$ (in the case where
$g$ is the identity, this is $E[U]$). In both formulations, there will be clear cases where the 
transformation function is unreasonable and, if risk aversion is rational, cases where it is reasonable. And one
can combine the two formulations for an even more complex attitude to risk, by maximizing 
$\int_0^\infty g(P(f(U)>\lambda)) \, d\lambda$ in the case of a non-negative $f$.}

While in the case of preferential treatment, utilitarianism provides a neat but implausible solution, so too in this case, expected utility
...
Second, goods may become significantly less valuable as they are repeated, decreasing in such a way that the sum of the values of any 
goods you could have over an arbitrarily long life has an upper bound.

But the repetition thesis is most plausible when boredom and other memory-based phenomena are in play. Suppose you have 
...
the most delicious and wholesome dessert ever prepared by the best chef on earth, a dessert  which you are told you've eaten some large 
...
$\sum_{n=1}^\infty n^{-2} \Var[X_n]$, where $\Var[X]=E[(X-E[X])^2]$ is the variance of $X$~\parencite[p.~132]{ChungProbability}.}
...
$1/2+1/4+1/8+\dots=1<\infty$. The Borel-Cantelli Lemma~\parencite[Thm.~4.2.1]{ChungProbability} then says that almost surely you will win only a finite number of times
...
Granted, in more realistic cases of long runs of independent lotteries, we can expect expected utility maximizers
to do better.\footnote{See, e.g., \citebyname{ZhaoRisk} and critical discussion of this as an argument for
expected utility maximization in general by \citebyname{WilkinsonRisk}.} 
However the utilitarian claim isn't just that we should \textit{typically} maximize utility, but that we should 
always do so.  If we are to typically do so, then we will have a Mersenne question as to the grounds of the 
distinction between the cases where we should do so and the cases where we should not do so, and it is implausible
to think any neat formula captures the distinction.

In any case, it appears that expected utility maximization is not rationally required. But it is the only clearly non-arbitrary solution
...
actual utilities that would result from an action. \citebyname{RailtonConsequentialism} calls this ``objective
consequentialism'', as opposed to the ``subjective consequentialism'' of which expected utility consequentialism is
a special case.} will have a nice answer to this problem. But utilitarianism has many highly counterintuitive 
implications (see also Section~\ref{sec:act-util} below).
...
conclusion that it is better to be Socrates unsatisfied than to be a satisfied pig~\parencite[Chapter~2]{MillUtilitarianism}.
...
several problems. The most famous is Parfit's repugnant conclusion~\parencite[p.~388]{ParfitRP}. Any finite scenario full of highly
...
up to positive affine invariance: they can rescaled and shifted without changing anything. More
precisely, for any positive 
...
earth die out. Some anti-natalists may disagree about the intuitions regarding average human life~\parencite{Benatar1997}.} 
...
inaccessible facts about intelligent life outside earth. One can, of course, solve this by restricting the averaging
...
be rather complex and will have multiple free parameters---or even infinitely many, as when having to choose a 
concave transformation function as in \posscite{Ng1989} Theory X$'$. And then we can ask what grounds these parameters' having the values they do. 


The above assumed, for ease of modeling, that we could reduce the utility enjoyed by each individual to a number. This is itself unlikely. Our flourishing seems to consist of a number of incommensurable goods such 
...
repugnant conclusion. Replacing eighty years of a deeply fulfilled life by a million of barely positive
...
the trolley is permissible but obeying the dictator's orders is murder, while consequentialists assume 
...
killing~\parencite{FootAbortion,DelaneyCloseness}. But it is difficult to see why intentionally firing a gun that happens to be pointed in someone's direction should 
...
Another motivation for closeness theories is cases like the following variant of an example
of Mark Johnston~\parencite[p.~338]{DelaneyCloseness}:
\nitem{Arson}{A terrorist has hidden a bomb that, if not found, will kill a dozen people. 
	You drive the terrorist to the house where he correctly believes his elderly bed-bound grandmother is sleeping, and 
	you burn it down in
	order that the terrorist might think his grandmother was burned to death. Being afraid for his 
	other grandmother's life, the terrorist reveals where the bomb was hidden.}
Notice that nothing in your plan depends on the terrorist's grandmother getting burned to
death, or even being in the house---if she died of natural causes five minutes before you arrived,
your plan would have worked just as well. You just need the terrorist to \textit{think} 
she was burned to death. And the means
for getting the terrorist to think this is not the grandmother's death, but the house being
on fire. If the grandmother was miraculously transported by angels out through the back window,
that wouldn't harm your plan. Yet surely you did something very close to intentionally killing
the terrorist's grandmother, so close that it is about as wrong as intentionally killing her.

There is indeed something intuitive about closeness theories. However, on such theories we have a seemingly arbitrary 
parameter defining the forbidden degrees of closeness to intentional killing, and we have a Mersenne question about what
grounds the parameter's value. It is worth noting, too, that at least one of the parameters is actually numerical. Suppose
one knows, but the terrorist does not, that the grandmother had serious health problems and hence that there is a
probability $p$ that she was already dead. If $p$ is sufficiently close to $1$, there is no significant closeness 
between one's action and killing (assuming there is no one else in the house).  If $p$ is close to $0$, the action
is too close to killing. Where is the cut-off? One may suppose $1/2$ is a natural cut-off, but that does not seem
right. For instance, playing Russian roulette with the terrorist's grandmother in order to terrorize the terrorist
seems impermissible, too.\footnote{\citeasnoun{LiaoCloseness}
suggests a way out of the closeness problem by means of a pluralistic theory on which other considerations, 
such intentionally involving someone in a situation that is in fact harmful or using someone as a mere means,
are employed to argue that intuitively impermissible actions that are close to killing are in fact wrong.
The \textsc{Arson} case probably escapes Liao's suggestions. There is no intention to involve the grandmother,
and \textit{she} is not being used, since all that matters is that the terrorist \textit{thinks} she is in the house.
But it is likely that Liao's plural considerations will have seemingly arbitrary parameters, such as how harmful
a situation has to be to rule out the involvement of the innocent person, or how intimately involved a person has
to be involved for them to count as being used as a mere means.}

\citeasnoun{OBKoons} have, however, offered a view of Double Effect on which it \textit{does} matter whether we are dealing with 
...
killing.\footnote{\citeasnoun{OBKoons} actually claim that one \textit{intends} killing in such a case. This seems wrong. After all,
...
Additionally, the O'Brien--Koons account does not entirely escape Mersenne questions. An artifact has a particular
...
This is a version of the closeness view, perhaps similar to the \citeasnoun{OBKoons} version. 
...
utility is positive. We have already seen serious problems with this in Section~\ref{sec:risk} in regard to uncertainty, expected value
...
But we have also seen in Section~\ref{sec:preferential} that taking relationships into account has many apparently arbitrary free parameters. So 
...
\subsubsection{Ethics: Torture}\label{sec:torture}
...
The first is akin to proportionality in double effect: what degree of aggression licenses lethal self-defense. For instance, if an assaulting party clearly threatens only to cut off your 
hair, lethal self-defense is unjustified, even though such an assault would constitute actual bodily harm 
according to the British case of \textit{DPP v Smith} \parencite[p.~73]{McCombe2017}. On the other hand, 
if the assaulting party is likely to kill you, then lethal self-defense is surely permissible. 
...
relevant.  \citeasnounwithpages{AquinasST}{II-II Q64 A7}, for instance, held that a private individual is not permitted to intend the death of 
an assailant.}
...
corrected for the economic state of the victim) and length of imprisonment, unless it turns out---as we have already
notes is quite unlikely---that 
...
arbitrariness. What grounds it?\footnote{This argument is further developed in \citeasnoun{DurhamDissertation}.}
...
how is a constitution instituted? One theory is that it happens by the consent of the people, which can be understood
as either the people at the time of the original passage of the constitution or the people currently governed by
it. But obviously for any state
...
Some of the above questions only make sense in the case of a formal consultation process of a sort that has occurred rather
...
needs to allow for implicit consent or custom.\footnote{Famously,
\citeasnounwithpages{AquinasST}{I-II Q97 A3} holds that custom can create and abrogate law.} But there things become even less clear.
...
Similar issues come up for other accounts of the constitution problem, such as \posscite{Hart} ultimate rule of
recognition embodied in the customs of the officials. Who are the officials? What fraction of the officials 
can act differently before their dissent counts as undermining the custom? 

If our interest is ethics, we can avoid these problems by adopting philosophical anarchism, and denying that 
law has any genuine authority. 
...
Traditionally, at least the following three conditions have been required for a state to enter into a war justly: 
\begin{itemize}
...
Let's focus on (ii) and (iii). Note first that the conditions need to be combined rather than having independent cutoffs. If the harms threatened by the enemy are sufficiently large---say, a threat of large-scale genocide---a small chance of success is sufficient to justify fighting back, while if the threatened harms are small, then in light of the 
great harms of waging war, a larger chance of success is needed.\footnote{For more on the interaction of the conditions,
see \citeasnoun{Hurka2004} and \citeasnoun{AloyoChance}.}
...
of waging war and not waging war. I will discuss utilitarian solutions in general in more detail in Sections~\ref{sec:act-util}--\ref{sec:rule-util}. 
...
This intuition is a part of the general conviction that in just war-time decisions when fighting on the side of
justice, one prioritizes friendly non-combatants
and friendly combatants to enemy combatants, while where exactly enemy non-combatants fit in is more controversial 
as is the question of how one prioritizes between friendly non-combatants and friendly combatants~\parencite{KasherYadlinMilitary, KasherYadlinAssassination, MargalitWalzer, McMahanInBello}.
But the prioritization had better not be lexicographic: it is wrong to kill a million enemy combatants to save one friendly combatant.
...
Hurka writes:
\begin{quote}Judgments about proportionality in war cannot be made simply or
mechanically. Even if we have specified the types of good and evil relevant to these judgments, we must identify the specific effects of a given
war or act, which requires comparing that war or act with alternatives
that are merely hypothetical and can only be estimated given our available evidence. In addition, once the relevant effects are identified they
must be weighed against each other, which often involves some indeterminacy because they are of irreducibly different types. \parencite[p.~66]{Hurka2004}
\end{quote}
Given that Hurka is evidently right about the complexities involved, it is very implausible that there is a 
simple formula that gives the answer. Unless we want to embrace the ``indeterminacy'' that he advocates for,
it is very plausible that there are many parameters determining whether a war is just, parameters whose grounding 
needs to be explained. And even if there is indeterminacy, we need parameters determining when the matter is
determinate and when it is on, on pain of a regress of indeterminacies (for more on vagueness, see 
Section~\ref{sec:two-step-vagueness}, below).
...
could be other substances that could have a different chemical structure or a different number of protons and similar behavior~\parencite[Lecture~III]{KripkeNamNec1980}. Similarly, \textit{we}
...
However, as \citeasnounwithpages{DurhamDissertation}{Section~4.4} notes, it is plausible that a substance's nature already grounds many complex dispositional facts about the substance. Furthermore, it is also plausible that it grounds many non-moral normative facts,
...
(with the convention that we prefer an infinite string of $9$s at the end to an infinite string of $0$s),
...
number. First as in note~\ref{fn:encode}, we can assume that each of the real numbers is between $0$ and $1$,
and encode each as a decimal (with the same convention about trailing $9$s). If the $k$th number is
$x_k=0.x_{k1}x_{k2}x_{k3}...$, then encode all the numbers as 
$0.x_{11}x_{12}x_{21}x_{13}x_{22}x_{31}x_{14}x_{23}x_{32}x_{41}...$. In other words, we put after the decimal point all the digits $x_{ij}$ where $i+j=2$ in lexicographic order of indices (that
will be just $x_{11}$),
then all the ones where $i+j=3$ (that will be $x_{12},x_{21}$), and so on.}
 In other cases, perhaps, the information for a Mersenne question may be specified as an open subset of
...
body engages in electromagnetic interaction. Since any two real numbers can be encoded as one\footnote{See note~\ref{fn:encode}, above.}, it follows that
...
human nature is finite, since maybe we live in a discrete rather than continuous world~\parencitecfwithpages{PrussInfinity}{Chapter~8}, and
...
Thus, precisely those cases that we have seen to raise compelling Mersenne problems make pure Kantianism an implausible ethical theory.\footnote{There is some hope that combining Kantianism with Aristotelianism would help~\parencite{MaglioDissertation}, but then
...
literature on evolutionary debunking arguments~\parencite{PlantingaWarrant,KahaneDebunk,WielenbergDebunk}. Second, a theistic reader has an easy way out of the argument:
...
\subsection{Act utilitarianism}\label{sec:act-util}
...
Mersenne questions, it shifts the bump under the run to others. As even \citeasnoun{MillUtilitarianism} has 
noted, for utilitarianism to be plausible and not swinish requires a hierarchy of goods, but then 
there will be Mersenne questions regarding the ordering in the hierarchy.
...
in the sense of desire-fulfillment utilitarianism or not? Reflection the complexity of human life, e.g., as seen in literature~\parencite[pp.~249--250]{AllenGeometry}, shows that there are likely
...
(see Chapter~\ref{ch:mind}.\ref{sec:functionalism}), and thus we will have a number of additional Mersenne questions. 

\subsection{Rule utilitarianism}\label{sec:rule-util}
...
to make the connection between our beliefs and the truth be sufficient for knowledge, but there is more hope here
...
with respect to maximizing utility. But act utilitarianism has already been put aside in Section~\ref{sec:act-util}.
...
ignorance~\parencite{RawlsJustice}. Anyone who has been in a long committee meeting knows that actual agreement between agents can result in 
...
The risk and uncertainty point is worth emphasizing. Rawls would say that rational agents would only agree to 
rules that do not treat anyone inhumanly. But a rational agent who is more accepting of
...
will still raise some Mersenne problems. For, as \citeasnounwithpages{AquinasST}{I-II Q90 A4} argues, legislation only has a claim on our obedience when it 
is ``notified to [those ruled by it] by promulgation''. And promulgation is a complex concept involving thresholds and parameters. It is not necessary
for promulgation that all those subject to the legislation have actually heard of it. But likewise it is not enough for the legislators to
...
\subsection{Brute necessity}\label{sec:brute}
...
the cardinality of the real numbers~\parencite{SolovayCanBe}. Suppose it turns out that in fact $CH_{15}$ is true.
...
question, much as it does in the ethical questions. Perhaps the truth of $CH_{15}$ could be a brute fact, albeit a necessary one since it is generally held that mathematical truths are necessary.\footnote{Though there is a 
controversial speculative Aristotelian metaphysical story on which some mathematical truths---perhaps even including things like the Continuum Hypothesis---are contingent~\parencite{PrussInfinities}}. 
...
One might object that the above argument against $(\pi,n,m)$-metaethics begs the question. After all, if the
metaethics is true, then to be obligated to $\phi$ just is for $\phi$ing to be commanded between the $n$th and
$(n+m)$th digits of $\pi$, and so that's what ethical normativity \textit{is}---the normatively significant
connection is \textit{identity}. But here is another way to see the point at issue. Moral obligation is not
the only normativity we have. We also have the normativity of emotions: different emotional attitudes are apt 
for different situations. Moreover, emotional normativity is hyperintensional. We can have propositions $p$ and
$q$ such that, necessarily, $p$ is true if and only if $q$ is true, but nonetheless we should have different
emotions towards them. For instance, an emotion of great wonder and delight is appropriate towards fascinating 
mathematical theorems like Fermat's Last Theorem, but not towards the fact that $48848\times 849=41471952$,
even though, necessarily, Fermat's Last Theorem is true if and only the multiplication fact is true (since both
are necessarily true). Now, it seems that the emotions appropriate towards the fact that Alice has performed a 
wrongful action are quite different from those that are appropriate towards the fact that Alice has performed an
action against which an injunction is encoded in $\pi$ between the $n$th and $(n+m)$th digits---indeed, quite
possibly there is no emotion appropriate towards the latter fact---even if the two facts are such that necessarily
each obtains if the other does.

We can use the normativity of our emotions to probe and criticize metaethical theories: a metaethical theory like 
$(\pi,n,m)$-metaethics would force an implausible revision in our emotional life. Similarly, an action's violating
a brute injunction in the Platonic heaven feels like something towards which we should have a lesser kind of negative
attitude than towards the action's being morally wrong. And that gives us reason to think being so brutely forbidded
is not what moral prohibition really is.

In other words, a metaethics where the ethical claims are grounded in something intuitively of no relevance to our moral activity, such as the content of the
digits of $\pi$, is not plausible. To be a candidate for a grounds of ethical claims, a thing needs to be ethically compelling---in a way that we should be able to \textit{feel}. For a more controversial
illustration of this point, consider \posscite{MacIntyreDivineCommand} argument that no collection of the traditional attributes of God is such 
as to make it plausible that the commands of a being with those attributes are what ethics is, and this is a strong reason to doubt divine
...
God include that God has created us and is perfectly good which might be held to ground obligations\footnote{Cf.\ \posscite{EvansGodMoral} 
gratitude-based account.}, if one were to replace God with a God$^*$ who has all the attributes other than goodness and lovingness\footnote{This is a \textit{per impossibile} conditional if the other attributes entail goodness or lovingness, as \citeasnounwithpages{SwinburneGod}{p.~13}, for instance, has argued in the case of goodness.}, the point would be even more plausible. Divine$^*$ command metaethics would clearly be unsatisfactory
...
nature is, we can think of this for now as a variant on the Aristotelian story without individual forms---a variant 
that will be more carefully evaluated in Chapter~\ref{ch:X}.\ref{sec:indforms}.

\subsection{Informationally complex natural universals}
The property or universal \textit{being a natural number} appears quite natural, in the \posscite{LewisNewWork} 
sense that is opposed to gerrymandered properties like \textit{being a dog or a book}. Yet we learn from
G\"odel's First Incompleteness Theorem~\parencite[Thm.~15.6]{BoolosJeffrey1995} that there is no way to
capture the truths about natural numbers in a finite (at least first-order) way. Thus a universal
may be quite informationally complex despite not being gerrymandered.

Perhaps, similarly, there is a natural universal that is nonetheless highly informally complex and that
captures the ethical phenomena. This could be a thin property like \textit{obligation} or \textit{virtuousness},
or a thick property like \textit{love}.\footnote{I am grateful to Charity Anderson for the last suggestion.}
Then ethics could perhaps be captured by a simple rule, like ``Do what's obligatory'' or ``Love
everyone'', which has a vast richness of entailments due to the complexity in the referenced universal.

As noted earlier, many of our Mersenne questions involve an appearance of contingency as well as pointing
to kind relativity. It seems very plausible, for instance, that the parameters in our initial example of preferential treatment are highly dependent on the kind of rational animal we are talking about. Rational animals that are more
dependent on their parents in childhood would likely be such that they ought to have a greater preference for
their parents over strangers than rational animals that have little contact with parents after procreation.
And what one says about rational animals that reproduce in ways radically different from us---asexually or 
with more than two individuals involved in reproduction---is likely going to be quite different. 

On this Platonic view, there is a vast number of apparently arbitrary parameters, such as that dragons
ought to have a preference of degree $d_D$ for parents while humans one of degree $d_H$. These parameters
have an appearance of contingency, and yet there is no contingency here at all---the necessary Platonic facts 
about universals fix them. But we can still try to explain the appearance of contingency as follows.
There are infinitely many possible humanoid kinds, and within some range appropriate to humanoids any 
possible degree of preference for parents will be the right degree for one or more humanoid kinds. And it is
contingent that the specific kind of humanoids that is actually exhibited on earth is, say, $H_{1249}$, so it 
is contingent that we have humanoids with a degree of parental preference $d_{H_{1249}}$, though it is not 
contingent humans---i.e., $H_{1249}$s---have that degree of parental preference.

Indeed, this is very much what the Aristotelian will say. It is necessary that we humans have the exact degree
of parental preference we do, but earth could easily have had a subtly different kind of humanoid instead.

There is, however, a relevant difference between our Platonic and Aristotelian views. On the Platonic
view, the kinds of rational beings will not be defined in terms of the ethical norms applicable to them,
since otherwise we have essentially the Aristotelian view again.
When members of a kind $K$ are obligated to have a preference of degree $d_K$ for parents, the kind $K$ is not 
defined with respect to the degree of preference, nor presumably with respect to any ethical normativity
(and maybe any normativity at all). In particular, we have supervenience of ethical properties on non-ethical
properties. And while this supervenience has been a part of the orthodoxy in philosophical ethics in the past century, the
vast amount of detailed Mersenne questions and apparently arbitrary parameters makes such supervenience 
implausible. For it is implausible to think there is a rule that maps the non-ethical features of 
a rational animal to a unique degree of preference for aiding parents and that does not involve apparently
arbitrary parameters of a sort that produce an appearance of contingency.

On the Aristotelian view, on the other hand, the kind $K$ is defined by the form, which intrinsically
encodes ethical and other norms. Thus, we can explain the appearance of contingency in the parameters
ethically governing our lives by noting that there are other humanoid kinds---likely including ones that
are biologically just like we are---whose ethical norms are different. This is akin to the Kripkean
explanation of the appearance of contingency in water being H$_2$O: something else with the same
``watery'' behavior could fail to be H$_2$O. We need not suppose a single necessary mapping from
the non-ethical features of a kind to the ethical features.

And while rules like ``Love everyone'' appear initially simple, if we realize that there is a vast collection of
necessary truths about love that involve apparently arbitrary parameters or rules that cannot be 
simply derived from a few elegant principles, the appearance of simplicity becomes less of an advantage
over an Aristotelian rule of ``Follow your nature'', where in fact your nature is in fact very complex. 
The Aristotelian rule is intrinsically simpler because the concept of a nature or form as such is less 
informationally complex than that of love---though, of course, your actual nature is highly informationally 
complex. (The concept of a book as such is not very informationally complex, but a given book may be 
highly informationally complex.) The simplicity in the Aristotelian rule is like the simplicity in a silly consequentialist rule like ``Maximize the universe's average temperature''---all the concepts in the rule are 
fairly simple, though of course facts about what actions fall under the rule depend on very complex features of 
the actual universe,
just as facts about what actions fall under ``Follow your nature'' depend on very complex features of 
your actual nature.

Additionally, it's worth noting that some of the attractiveness of the Platonic approaches disappears on
closer examination. It is appealing to think that one can explain all of ethics in terms of one universal,
or perhaps universal per kind, and a simple rule like ``Do what what's obligatory'' or ``Love everyone.'' 
However, such proposals leave out a significant portion of ethical discourse. Granted, figuring out what
we are obligated to do is indeed ethically central. But obligations come in degrees of weightiness. I don't 
mean by this that weightier obligations override less weighty ones. Rather, some obligations are worse
to break than others. Under ordinary social circumstances, it is equally true that I am obligated not to slap
you in the face and that I am obligated not to shoot you in the kneecap. But it is worse if I shoot you
in the kneecap---that violates a weightier obligation. 
One can try to ground comparisons between obligations in terms of obligation \textit{simpliciter} by 
noting that some obligations override others. Thus, if an evildoer tells me that they will kill you
unless I either slap you in the face or shoot you in the kneecap, I should slap you in the face. 
However, the question was what grounds the fact that I am more weightily obligated to refrain from 
shooting than from slapping under ordinary circumstances, and it does not seem right to ground
this in my obligations in extraordinary circumstances. And, in addition to obligations, there are moral
reasons of varying force, including ones generating cases of supererogation. 

Thus, the theory will likely require some universals other than the binary relation of obligation
between an agent and an action, such as a ternary comparative obligation relation between an agent and two 
actions, or a ternary moral-reason relation between an agent, an action and a force. 
Similarly, in the case of love, there are actions that are more loving than others. These will include
loving actions one of which is more loving than the other as well as unloving actions one of which is,
nonetheless, more loving than the other. It does not seem possible to define comparisons between degrees of 
love simply in terms of the predicate ``loves''. We will need universals corresponding to the quaternary 
relation expressed by the sentence ``$x$ $\phi$ing is more loving towards $y$ than $x$ $\psi$ing''.

\subsection{A two-step vagueness strategy}\label{sec:two-step-vagueness}
...
(i)~non-classical logic, (ii)~supervaluationism and (iii)~epistemicism.\footnote{Elizabeth Barnes' classical-logic
version of ontic vagueness~\cite{BarnesOntic} is difficult to classify. On this view, every proposition is either
true or false, and indeed it is determinate (i.e., non-vaguely true) that it is true or false, and logic is classical. 
It is also true that we do not know whether a vague proposition is true or false. So far this sounds like 
epistemicism. After all, it follows that in a Sorites sequence starting with a hairless person and moving through
more and more hirsute people until we get a person with a paradigmatic full head of hair there \textit{is}
some first non-bald person. However, Barnes also  insists that the world does not ``settle'' whether a vague proposition is true 
or false. This seems problematic. Suppose Jim determinately exists but is vaguely bald. Then we can say that either it is true that 
Jim is vaguely bald or it is false that Jim is vaguely bald. It follows by classical logic that it is 
true-and-not-settled that Jim is vaguely bald or it is false-and-not-settled that Jim is vaguely bold. 
Since a proposition is settled if and only if its negation is settled, it follows that one of these
propositions is true-and-not-settled: that Jim is bald or that it is not the case that Jim is bald. In
either case, there is a true-and-not-settled proposition. But it is difficult to see how a proposition
could be true, and hence true at the actual world, and yet not settled by the actual world.}

It is 
difficult to make sense of what it is for a proposition to be not settled by the world and yet true, and 
...
The non-classical approach to vagueness~\parencite[Chapter~4]{WilliamsonVagueness} will involve a logic with one or more additional truth values inserted
...
feature of the logical model of vagueness and what matters is their ordering~\parencite[p.~132]{WilliamsonVagueness}. And
...
with ``precisifications'' or ``interpretations''~\parencite[p.~294n4]{WilliamsonVagueness} corresponding to different classical truth values. ``Bob is bald'' may be
...
one raised in Section~\ref{sec:brute} regarding $(\pi,m,n)$-metaethics: it would be arbitrary to require obedience to the commands that are found starting with the $m$th digit of $\pi$,
...
we saw in Section~\ref{sec:torture} that what kind of infliction of pain is absolutely wrong raises Mersenne questions. But 
in the supervaluationist cases, it will then be a merely verbal or ``non-substantive''~\parencite[Chapter~4]{SiderWriting} question whether one has a certain ``reason''---it depends
on the precisification. But to ask that people die for what is ``purely verbal'' is
obscene, and to die oneself for such a ``reason'' is a failure to respect one's own humanity (to put it in Kantian
terms).

Finally, consider a standard epistemicist theory of vagueness according to which there is a true semantic theory that assigns to each term the precise meaning it has in the
light of the patterns of our use of that term, but neither that semantic theory nor the empirical data on the patterns of language use are available
to us in sufficient detail to settle the meaning of vague terms~\cite[pp.~231--232]{WilliamsonVagueness}.  Thus, there is a precise fact as to how much hair one can have
...
or the right, rather than by some variant property, would be entirely arbitrary. As \citeasnounwithpages{SchoenfieldVagueness}{p.~265--266} observes, it is absurd
to think that linguistic usage settles the question of the permissibility of abortion, and adding knowledge of
the correct semantic theory to the facts of linguistic usage should not help either.\footnote{\citeasnoun{HawthorneMoralVagueness} criticizes Schoenfield by observing that it is not enough on epistemicism to know the patterns of linguistic usage---one
also needs to know the way that the semantic facts depend on the patterns of usage.} 
...
epistemicism as applied to ethics turn ethical questions into purely verbal ones, undercutting a robust 
moral realism.

\citeasnounwithpages{HawthorneMoralVagueness}{p.~218} notes that one can imagine
a continuum of patterns of linguistic usage, across possible worlds, on which ``morally good'' shades between the meaning it now has and its
meaning \textit{made of gorgonzola cheese}. Plausibly there is a possible world along that continuum where
``morally good'' refers to the morally good but does so in a way that is indeed epistemically vague in the Williamsonian way, because it depends on fine details
of usage and semantic theory whether in that world ``morally good'' refers to the morally good (as it does) or 
to the gorgonzolan. 
Thus I should not overstate my case by claiming that a term that refers to the morally good could not be
epistemically vague. However, such a world would not be one where the concept expressed by the term ``morally good'' 
would legitimately play the kind of central normative role that the concept of the morally good plays in our lives
(unless the denizens had some other and non-vague way of expressing the cocnept).
Indeed, it would not be right for us to say that the denizens of that world know that what falls under their 
concept of the good is indeed supremely worth pursuing (since one does not know borderline cases on
epistemicism), even though in fact it \textit{is} supremely worth pursuing. Interestingly, it
might even be the case that in their variant of English ``The morally good is always worth pursuing'' is non-vaguely
true. For ``worth pursuing'' might march semantically in lockstep in the continuum of worlds along with the ``morally 
good'' in such a way that in worlds where ``morally good'' means \textit{made of gorgonzola}, ``worth pursuing''
means \textit{made of cheese}. This thought experiment suggests that it may not be enough to characterize our
robust moral realism by saying we \textit{know} central moral truths, such as that the morally good is always worth
pursuing, but we need to be committed to there not being an undermining outside view, like the one we have with regard
to the world where ``morally good'' is semantically next door to meaning \textit{made of gorgonzola}. How exactly 
to state this constraint on moral realism is a difficult question, but it does seem that something has gone seriously
wrong in a world where people sacrifice their lives for the sake of a property that for aught they know is identical
to \textit{made of gorgonzola}---that is just not a world where robust moral realism is true.

In this critique, I was following the standard understanding of epistemicism as grounding the vagueness in unknown
patterns of use and an unknown mapping from patterns of use to meanings~\parencite[pp.~231--232]{WilliamsonVagueness}, so that if moral properties are vague there are no privileged moral properties. There is, however, a more general understanding of epistemicism as
...
as we will in Chapter~\ref{ch:meta}, and if we grant that we could have aspects of our physical nature whose teleology is
...
in other normative areas, such as epistemology (Chapter~\ref{ch:epistemology}) and semantics (Chapter~\ref{ch:semantics}), and the cost of error theory 
...
standard objections. Perhaps the most obvious is that it justifies an ultra-conservative approach: for if what
...
will have more Mersenne questions. There is a complex and difficult literature on how to attribute doxastic states to a community~\parencite{Quinton1976, Hakli2006, deRidder2022, Lackey2020, Tuomela1992, Gilbert1987, Gilbert2006}. A reasonable
...
high rates of obesity. By reflecting on our nutritive instincts and \textit{other data}---such as the teleology of
...
\section{Epistemic underdetermination}
One of the main critiques of Aristotelian approaches to ethics is that the empirically observable facts 
epistemically underdetermine judgments about proper function and teleology~\parenciteeg{LewensUnder}. 
There are many cases where it is not clear whether something is a mere matter of natural variation
or whether it is an abnormality, and as debates in the philosophy of disability show, in the case
of humans such questions can be deeply fraught with human significance. But the questions also come
up in the case of non-human organisms. 

But unless we implausibly think that \textit{no} judgments of proper function are epistemically justified---yet surely 
thinking that oaks ought to photosynthesize and that humans ought to eat is justified---epistemic
underdetermination is what we should expect on our robust Aristotelianism~\parencitecf{LockhartLott}.
For the metaphysical
robustness of our Aristotelianism makes us expect facts about form not to supervene on the
empirical facts about the actually exhibited behavior of organisms, and where there is no supervenience on the
empirical, we should expect to have some cases of epistemic underdetermination as well. Compare how on a 
non-naturalistic view of mind we should expect there to be cases where we cannot tell on empirical grounds
whether some entity is conscious, and indeed cases where our inability to figure this out is likely to be
insuperable. And indeed I expect the question of whether lower animals and plants are conscious is insuperable.

Now, one might think that ethics is different from other areas. Perhaps in ethics we should expect to be able
to figure out all the questions. But the large number of apparently arbitrary parameters we find in our moral
norms, many of them highlighted earlier in this chapter, make such a Pollyannaish expectation of epistemic
determination of moral facts by empirical ones implausible. 

Thus, epistemic underdetermination considerations seem to be if anything evidence for our robust
Aristotelianism.
Nonetheless, an \textit{optimistic} Aristotelianism predicts that we can figure out the relevant judgments
in a number of cases. And this, too, seems to be what we observe. For more discussion of the optimism here,
see Section~\ref{ch:God}.\ref{sec:epist-of-form}.
...
go, though it leads to further questions that will be examined in Chapters~\ref{ch:details} and~\ref{ch:God}. The major alternatives all
