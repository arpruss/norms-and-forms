...
in the sequence being extremely slight. What makes for the transition?

The most popular answer is to reject the presupposition that a new macroscopic object
pops in somewhere along the sequence, by accepting either mereological nihilism, which holds that
there are no complex objects, or mereological universalism, which holds that every plurality of
objects composes a whole. Indeed, \citeasnoun{Sider4D} originated the continuous series 
thought experiment precisely to argue for mereological universalism. 

Nihilism implies that either we are
simple or we don't exist. That we don't exist is obviously quite implausible (\textit{pace} \citeauthor{UngerDoNotExist}, \citeyear{UngerDoNotExist}). 
The main view on which we are simple is a Cartesian dualism on which we are souls. This has
well-known counterintuitive consequences, such as that we have never seen other people. But
most seriously, Cartesian dualism has implausible ethical consequences~\parencite{LeeGeorge}. If bodies are not a
part of us, it is difficult to justify the deeply morally relevant distinction between property damage
and bodily harm~. And it is difficult to build a plausible sexual ethics on a view on which our sexual
organs are not parts of us. Moreover, on mereological nihilism, even if one can use Cartesian dualism to salvage
...
``neighbors'' in the case of a larger person~\parencitecf{Johnston2016}. If only I am conscious, then we have the
...
$p(w)$ is non-empty and has $p$ as a mereological modal profile. If we accept mereological
...
would be a set of all cardinalities $\kappa$, and there is not~\parencite{PrussCardinalityObjection}. But surely 
...
Responses can doubtless be given to these and other problems, but at least we can say that there are
...
However, the Aristotelian has a solution, namely
the invocation of form: it is not possible to have a sequence where at
...
somewhere in the sequence, and that is where the metaphysical transition happens.
...
composes a whole~\parencite{vanInwagenMaterialBeings}. The Aristotelian answer violates a stricture of van Inwagen's, namely that the answer be informative,
...
beings, God is creatively involved (see Chapter~\ref{ch:God}).
...
seem right. On a natural Aristotelian interpretation of the metaphysics of conjoint twins, there are common
...
important to give an account of diachronic identity---maybe it is brute~\parencite{MerricksBrute,PrussIdentity}---but it seems that
...
practices of promising, rewarding and punishing, and prudential reasoning 
all presuppose personal identity over time. In the case of diachronic \textit{personal} identity, however,
we do have four main substantive alternatives to \dref{identity-Arist}. On the brain and soul theories, the
identity of persons is defined by (functional or living) brains~\parencite{CampbellMcMahan} and souls~\parencite{SwinburneSoul1997} respectively.\footnote{One may worry that 
...
theory, the identity
of persons is biological continuity of organisms~\parencite{OlsonHuman,ThorntonVarieties}. On psychological theories, the identity of
persons is defined by psychological continuity of mental life~\parencite{ShoemakerIdentity,ParfitIdentity}.
...
also \posscite{Johnston2016} personite problem.} 
...
on animalism~\parencite{OlsonWhat,OlsonWhat}. The simplest version is that if your cerebrum is removed from your skull and placed in a vat in such a way
...
Aristotelian theories). This does not 
...
to the tragic real-life scenario of upper brain death. Bioethicists disagree on whether upper brain death is death~\parencite{TruogDeath,NapierDeath}.
...
functioning characteristic of humans. But that we started our life as beings much smaller than zygotes invites 
...
\subsection{Of snakes, rattles, form and matter}\label{sec:rattlesnake}
...
\subsubsection{Non-living and living substances}\label{sec:living}
...
Granted, it is counterintuitive to think of a plant that does not (yet or any more) have a single atom. But let's take
...
in the way artworks do. As we saw, moral significance tends to push one away from positing vagueness (Section~\ref{ch:ethics}.\ref{sec:sec:two-step-vagueness}), and so there is something
attractive about the idea of there being a sharp distinction between life and non-life, and seeing life
as grounded in a special type of substantial form, a \textit{soul}, is thus at least somewhat appealing (see also
Section~\ref{sec:life}).
...
compelling. And as we saw in Chapter~\ref{ch:mind}, there is good reason to think on the basis of the shortcomings of 
...
body, and we might end our lives as subatomic objects in a big toe (compare the rattlesnake argument, after all, in Section~\ref{sec:rattlesnake}).
...
It has been commonly believed by Aristotelians that in order to receive a form, matter must be properly disposed, and,
...
question. Start with an artifactual example. Famously, Michelangelo in his Sonnet 151 holds that statues are already there in the 
stone, but need to be freed~\cite{Nims}. It seems more in keeping with common sense ontology, however, to say that the statue came 
...
It was essential to the normative applications of Aristotelian forms (Chapters~\ref{ch:ethics}--\ref{ch:epistemology}), and the philosophy of science
considerations (Chapter~\ref{ch:laws}), that we understand
...
necessary and sufficient conditions for a plurality of entities to compose a whole~\parencite{vanInwagenMaterialBeings}.
Recall that the two extreme answers are nihilism, which holds that a plurality of two or more objects never 
composes a whole, and universalism, which holds that every plurality of objects composes a whole. All other answers are  restricted theories of composition, including the Aristotelian answer defended in Section~\ref{sec:comp}.
...
has to be such as to be applicable to things the size of a small molecule. Van Inwagen's answer~\parencite{vanInwagenMaterialBeings} that the $x$s compose
...
This is a generalization about the causal powers of things, rather than a metaphysical principle of ``existential inertia''~\parencite{SchmidInertia}.
...
As we saw in earlier chapters, it grounds normative and nomic facts. And as our relativistic arguments show, the form cannot
...
by saying that the human form was a soul with an abstract intellectual function that did not depend on a body~\parencite[I, 75, 2]{AquinasST}. This response may 
...
For consider both the moral and the intellectual aspects of our flourishing. As Aristotle himself thought, true virtue is 
rare\footnote{For instance, he states states that ``the many'' avoid wrongdoing merely from fear of punishment rather than for the sake of what is fine and noble~\parencite[10.9]{AristotleNE}.} Indeed, when we look at ourselves, we see that all too often even actions that are right do not flow from motives far from 
...
emphasized to little Feynman that we have no idea why they do that~\parencite[16]{FeynmanWhat} Our scientific questions bottom out with laws of nature,
...
\section{The definition of life}\label{sec:life}
...
with most versions of naturalism. In Chapter~\ref{ch:God} I will argue that the best version of the Aristotelian account
will be theistic.
