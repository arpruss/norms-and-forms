...
Here is one highly speculative way to answer the questions. We can connect reasons and norms with goods. We might, for instance, say 
that an (internal) reason for a behavior is a representation of the behavior as good, and then we could say that a necessary 
condition for a behavior being voluntary is that it comes from such a reason. The will then could be functionally defined as
the system by which reasons lead to behavior, and a rational norm could then be a norm governing which of the reasons 
that one has are to be acted on. We might then say that a moral norm is a rational norm that favors reasons concerning goods
not considered as goods to self as such.??
