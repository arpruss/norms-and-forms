...
\ditem{benefit-even}{Benefit the ones with ticket numbers $2,4,6,...$}
\ditem{benefit-odd-1}{Benefit the ones with ticket numbers $1,3,5,...$}
...
\ditem{benefit-odd-3}{Benefit the ones with ticket numbers $3,5,7,...$}
with the very same benefits. 
Observe now that \dref{benefit-odd-3} benefits people whose ticket numbers are one plus the ticket 
numbers of the beneficiaries of \dref{benefit-even}, while \dref{benefit-even} benefits people whose 
ticket numbers are one plus the ticket numbers of the beneficiaries of \dref{benefit-odd}. Thus,
...
ticket number $n$ as $(n-1)^*$, then options \dref{benefit-even} and \dref{benefit-odd-3} are equivalent to:
\begin{itemize}
...
is a little bit more costly than the other. Moreover, we would like to have a principled account of when 
incomparability occurs. And it is difficult indeed to find one. The most radical account is that giving an 
equal benefit to sets $S$ and $S'$ of people is comparable when and only when one of the sets is a subset 
of the other, with the obvious comparison (it is better to benefit the people in a strict superset). But this has the 
implausible consequence that if there are two people drowning to starboard and 
one to port, and you can launch a lifeboat on only one side, neither option is preferable---but surely saving
the two is better. 

We can escape this problem by saying that giving an equal benefit to sets $S$ and $S'$ 
of people is comparable when one set is a subset of the other or when one set has a greater cardinality of 
members than the other, with the obvious comparisons in those cases. This has a counterintuitive consequence
for combinations of actions. Let $S$ be the set of people with tickets $1,2,3,...$, 
and let $S'$ be the set of people with tickets $0,4,8,12,...$. On the present benefiting the people in the two 
sets will be incomparable, since the two sets have the same infinite cardinality, and neither is a subset of the
other. But note that the decision between the two sets can be decomposed as follows. Let $T$ be the set of people
with tickets $4,8,12,...$. Let $U$ be the set of people with tickets in $S$ that are not in $T$ (i.e., positive 
numbers not divisible by four), and let $V$ be the set consisting of the person with ticket $0$. Then 
benefiting the people in $S$ is equivalent to benefiting both the people in $T$ as well as the people in 
$U$, while benefiting the people in $S'$ is equivalent to benefiting both the people in $T$ as well as the 
people in $V$. Since it is better to benefit the people in $U$ than those in $V$ on our current account (there 
are infinitely many in $U$ and only one in $V$), it seems like the decomposition should make benefiting the 
people in $S$ be better than benefiting those in $S'$, contrary to the account.

We might try an even more complex account. Perhaps we prefer benefiting the people in $S$ to those in $S'$
provided there are more people in $S-S'$ than in $S'-S$, where $A-B$ is the set of entities in $A$ that are not
in $B$, we are indifferent if the cardinalities of the two sets are finite and the same, and otherwise we have 
incomparability. This nicely handles the problem in the previous paragraph: $S-S'$ in the previous paragraph's 
example has infinitely many members while $S'-S$ has only one. ??transitivity?? -- yes!!!! oops! Hmm, so maybe
this works?

This has the somewhat counterintuitive consequence that benefiting the people 
with a positive odd-numbered ticket and benefiting the people with a prime-numbered ticket are incomparable
because although almost all of the people benefited in the second case are benefit in the first case,
and infinitely many more 

Here is another way to see a problem. Suppose a line stretches to infinitely in both directions,
...
\ditem{benefit-reflect}{Giving an equal benefit to the people standing at the points of a set $S$ of 
tickmarks is on par with giving the same benefit to the people standing at the points of $S_x$, for any $x$.}
Moral value should be invariant under mirroring.

But just as intuitively intuitively:
\ditem{benefit-subset}{If $S'$ is a proper subset of a set $S$ of tickmarks, then giving the same benefit 
to the members of $S$ is better than giving it to the members of $S'$.}
Benefiting some extra people is always good.

Unfortunately, \dref{benefit-reflect} and \dref{benefit-subset} can be easily proved to be inconsistent with the 
...
mirroring $-2$ about $1$ gives $4$, and so on). By \dref{benefit-reflect}, benefiting the people in $S$ is 
on par with benefiting the people in $S_0$, which is then on par with benefiting the people in $(S_0)_1$.
But because $(S_0)_1$ is a proper subset of $S$, by transitivity this contradicts \dref{benefit-subset}.
