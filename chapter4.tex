...
In general, utilitarianism, contractarianism and most versions of Kantianism\footnote{But see \citeasnoun{MaglioDissertation}.} focus on the agent's rationality, taking the details of the agent's humanity to provide no direct normative
...
it may be that the very concept of a human is normative.}, and the normativity of the
...
a specific degree (given the discussion in Section~\ref{ch:ethics}.\ref{sec:preferential}, this option seems better). The laws could even have social construction built in: they could require us to respect our relatives in the ways that our society prescribes,
...
A more sophisticated theory may combine aspects of the social and individual theories, holding that not only do couples undertake obligations to each other and gain
...
words when they get married, and the words often do not do justice to the nuance. Reasons of beneficence have 
different weight depending on the agent's relationship to the
patient (cf.\ Section~\ref{ch:ethics}.\ref{sec:preferential}). One of the things that is discovered in marriage is facts about the weight that the marriage relationship
...
we come to see that it is H$_2$O and by observing mammals, we come to see that their middle ear should have the malleus, incus and stapes bones. However, we are often best positioned to observe marriage at work in our own case, so it 
...
she was rightly referred to as a ``queen'', but obviously she did not have rights and obligations of a monarch with respect to the United States, and so ``queen''
...
arguments that extending marriage to same-sex couples does no harm to opposite-sex couples~\parencite{TwentyYears} will ring hollow. For it is a part of the proposal that the
...
available. For instance, perhaps there is some reason for colleges to limit certain sports such as gridiron football to one gender, but if so then they should make other sports available
to persons of other gender~\parencite{FrancisSports}.
...
allows for an elegant solution to the problem of what the task of medicine is.
...
to refuse a commission that would be unavoidably kitschy. And it is especially true of the medical professional
that they are not mere providers of legally permissible services requested by a client~\parencite{CurlinTollefsen}.
A medical professional should reject a patient's request to have a toe removed from each foot simply because the patient
wants to use the number of toes on each foot express their disagreement with the ancient Pythagorean preference for odd numbers over even ones.
...
having a very sharp focus on health.
 Medical considerations are of great importance to everyone's life. But in
...
decisions that medical professionals understand health holistically, rather than narrowly thinking only of kidneys or feet. Thus, a focus on health in general
...
Health cannot be defined purely statistically---it has a normative component~\cite{LennoxHealth}. 
One way to see this is that there is a directionality in health. Having muscles capable of deadlifting
four hundred kilograms and having muscles incapable of deadlifting four kilograms are both significant
deviation from the average for an adult human, but only the second is unhealthy. The ability to deadlift
a great weight might be said to be supernormal, but its possessor not thereby abnormal, nor do 
is there a need of medical treatment to reduce their strength, as in \posscite{VonnegutHB} dystopian 
``Harrison Bergeron'' story. 

One may try to respond to the deadlift example by focusing on the distinction between capabilities and incapabilities,
and saying that having a \textit{capability} of at least statistically normal degree or an \textit{incapability} of at 
most statistically normal degree is healthy. This is a neatly non-normative accunt, but it is unsatisfactory, since a significantly above
average capability to involuntarily collapse from a small impact to the head is not healthy. 
 Capabilities are not
always good or healthy.\footnote{One might think that voluntary capabilities are always good as they contribute to
the person's agency. However, voluntary capabilities are only a small subset of an organism's capabilities.}
Furthermore, the distinction between capabilities and incapabilities appears arbitrary. We can describe a
cell-membrane as having the capability to transmit some chemicals and the incapability of transmitting others
(and both can be healthy), but we can also describe it has having the incapability to block some and the capability
to block others.

Or one might go for a \textit{partially} statistical account of health as in \citeasnoun{BourseHealth}, where we have a non-statistical notion of the
directionality of function a bodily system---muscles are for contracting and skulls are for resisting impact---and
a statistical notion of what degree of function or readiness for function is normal. Health then involves 
function or readiness for function of \textit{at least} statistically normal degree. 

This still requires an account of the function of a bodily system. For Bourse, this is defined by the 
system's statistically typical contribution to individual survival and reproduction. This is implausible, however.
If a crazed dictator of the world were to institute a policy of slaughtering all four-legged sheep, 
three-leggedness would not become intrinsically healthy \parencitecf{PlantingaWarrant}, though it would 
typically contribute to individual survival. Furthermore, such a non-normative account of health makes
it unclear why the task of medicine is worth pursuing. For while survival and reproduction are very plausibly
good for a human being, health on this definition is defined by the \textit{statistically typical}
contribution of a putative function to survival and reproduction. But the question of whether a condition
\textit{typically} contributes to survival and reproduction is only evidentially relevant to the question
of whether the condition contributes to an individual's survival and reproduction, and it is surely 
the individual's survival and reproduction that we care about. The mere fact that a feature tends to
contribute to survival and reproduction in the population does not constitute the feature as choiceworthy when it 
does not do so in the individual, while health does constitute a feature as choiceworthy. On this account, 
then, by pursuing health we end up pursuing a mere proxy for what we care about.

One might alternately try to account for the function here in evolutionary terms as a number of authors
have done~\parencite{Wright1973,Millikan1984,Neander1991,Neander1995,Agar1993,Sterelny1990,Papineau1993,Dretske1995}.
The basic idea of these accounts is that a bodily system's function is the behavior which contributed to the 
phenotype having been evolutionarily selected for. However, besides technical objections one might make to such
accounts~\parenciteeg{KoonsPruss}, reducing health to conformity to evolutionary history
faces exactly the same problem as Bourse's account of function: there is little or no value in being healthy,
and hence what medicine pursues is only a proxy for what is valuable. For there is little if any value in having
bodily systems that function in ways that millions of years ago helped our ancestors survive and reproduce. 

Besides this, even if one can define healthy directionality in one of these non-normative non-Aristotelian ways, 
the question of statistically defining an abnormal deviation is highly problematic. Excessive sugar consumption 
tends to decrease heart function. Thus, on a statistical account, the soft-drink industry could bring about 
cardiac health in a number people with marginal cardiac dysfunction by promoting sugary drinks and thereby 
bringing down the population average or median of cardiac function---yet it is absurd to think that anyone
would become healthy as a result. Conversely, a government program to promote
exercise and thereby improve cardiac function, if successful, makes some non-participants get heart disease 
by shifting the average or median upward---and indeed without the consent of these non-participants.
This is absurd. Furthermore, the degree to which one must fall short of the statistical norm to count
as unhealthy appears quite arbitrary. Does health require the 40th percentile for heart function, the 20th, or 
the 5th? 

Instead, I take we should thus take health to be something through-and-through normative, a good of the body,
such that its lack is bad for the body.
At the same time, health is not simply \textit{the} good of the body. There are goods of the body besides health, such 
as athletic achievement, beauty, and actual reproduction. These goods depend on health, but are not a part of health: 
for instance, a relatively healthy reproductive system is needed for reproduction, but one can have such a system 
without actually using it to obtain the good of reproduction.

Among our goods, it is perhaps health that is most clearly species-relative. 
...
to be the only thing the norms of our will call us to promote---the flourishing of others seems just as 
important.
...
Unless we choose to limit the word ``moral'' to reasons having to do with other persons, these norms will then
...
\section{The definition of life}
...
The problem of the definition of life is a thorny conceptual problem in biology or its philosophy. Infamously,
different authors give quite different lists of empirically-focused features such
as homeostasis, growth, reproduction, information encoding and metabolism as part of the definition of 
life \citeasnoun{MalaterreChartier} use data-mining techniques to identify eight different categories
(the first three are metabolism, proteins/catalysis, and cellular/structural features), each
with many features under it.
The multiplicity of features listed makes the concept of life seem arbitrary. Indeed, Malaterre and Chartier's
conclusion is that life is a multidimensional degreed concept. But the Aristotelian approach may allow for a
metaphysical approach that escapes the multiplicity of empirically-driven categories.
  
Indeed, it is philosophically problematic to tie the the concept of life too tightly to the physical forms of life around us. For if there are immaterial agents such as deities, spirits or angels, they should also count as alive.
 After all, those who believe in such beings often
...
of life should include the kind of life that is attributed to immaterial agents, and typical accounts in the philosophy of biology fail to do that.\footnote{The closest in Malettere and Chartier's survey is information encoding. However,
it is unlikely that information encoding \textit{by itself} is enough to define life---an ordinary computer is
constantly encoding information without being alive. Moreover, it is not clear that information encoding can capture
the intellectual activity of immaterial agents. For instance, classical theism's doctrine of divine simplicity 
does not allow God's knowledge of contingent reality to be internal state encoding the external contingent reality---instead, perhaps, it is some sort of an extrinsic ``gaze'' \parencite{GrantSimplicity}.}
...
And while intelligent robots like Data are described as ``synthetic life forms'' in the \textit{Star Trek: Picard}
series, and the Star Trek canon clearly favors their being actually alive, the question is
...
admittedly, there may be some confusion in Star Trek between consciousness and life.} And
whether a virus qualifies as alive varies from definition
to definition in a way that makes it sound like the question of viruses being alive is merely verbal. Yet given the strong intuition that there
is significant value to life, even something sacred, the question of what is and is not alive should not be merely verbal.
...
artifacts only had form in the analogical sense of an art in the mind of the designer~\parencite[\textit{Metaphysics} XII.4]{Aristotle}, but he could have been wrong in the case of artifacts like Data.
(For more on the epistemic issues here, see Section~\ref{ch:God}.\ref{sec:epist-of-form}.)
...
a way that might well violate causal finitism, the thesis that it is impossible for an infinite number of causes to work together~\parencite{KoonsKalaam,PrussInfinity}. And among minded beings, perhaps it is definitive of \textit{persons} that they pursue all good.
