...
There are epistemic norms. Prescriptively, they tell us we should believe in accordance with the evidence, search 
for truth, avoid contradiction, and so on, while evaluatively, they tell us that some beliefs count as justified 
and others do not, that some beliefs are apt and others inapt, and so on. What \textit{grounds} the normative force
of these norms? 

There are three main types of answers in the literature. We have deflationary views on which epistemology
is not normative at all, but simply a kind of engineering-for-truth~\parencite[664--665]{Quine1986}, or,
somewhat more normatively, just a matter of analyzing the meanings of words like ``should'' or ``justified'' 
in non-normative terms such as reliability~\parencite{Goldman1986}. We have value-centered views on which
the force of the norms comes from the value of the epistemic goals, whether subjective~\parenciteeg{Kornblith1993} or
objective, such as truth.
And we have virtue-centered views on which the norms are constituted by what epistemically virtuous agents
do~\parenciteeg{SosaVirtue}. 

None of these are entirely satisfactory as they stand. And our robust Aristotelianism provides an attractive meta-epistemology that
is either an alternative or complement to the ones in the literature.
Human nature, in defining human flourishing, defines the flourishing of our intellect as such. Positive epistemological
evaluations for humans are then grounded in the flourishing of the human intellect, and negative ones in the human intellect's languishing, and similarly for non-humans with intellects, if there are any such.
An apt belief could then be defined as one whose possession constitutes the flourishing of the intellect. 
An aptly acquired one is one whose acquiring constituted the flourishing of the intellect. And so on. ``Why should we think rationally?''\ is answered
in the same ways as ``Why should we act morally rightly?'': failure to do so is failure at being a well-functioning 
human.\footnote{Compare \posscite{BrogaardFlourishing} norm of belief on which one must believe only that which
does not hinder intellectual flourishing. It is possible for an Aristotelian to simply provide a complement
to Brogaard's account: the human nature is what explains what constitutes the intellectual flourishing that Brogaard
thinks is not to be hindered. However, it seems better to focus explicitly on what is constitutive of intellectual
flourishing rather than on what does or does not hinder it. Take the example attributed by Xenophon to
Socrates where one is kidnapped to serve as an advisor to a tyrant because of one's 
wisdom~\parencite[\textit{Memorabilia} IV.2]{Xenophon}. It may well be that serving as an advisor to a tyrant
will be intellectually corrupting, and hence will hinder one's intellectual flourishing. But surely reasoning
wisely is epistemically right even if in fact it will result in such a kidnapping. This is obviously true
in the case where one has no reason to expect to be kidnapped by a tyrant, and that is enough to be a counterexample
to Brogaard. But even in cases where one expects such a kidnapping, it seems plausible that it is epistemically
wrong to reason unwisely to avoid the kidnapping, much as it would be morally wrong to steal money for 
tuition in an ethics class that would on the whole improve one morally.}

 but it is likely also true even
when such a kidnapping is to be expected, for the same reason as that one should act virtuously even if one
expects that virtuous action will 

Deflationary views largely leave unanswered the ``Why should we think rationally?''\ question.
Subjective value-centered ones give a wrong answer: someone who wants to maximize the number of fallacies
they have fallen into on Tuesdays afternoons (or is a member of some community organized around such a 
goal) thinks irrationally while committing the fallacies. 
Objective value-centered views are more promising. It is plausible that I should follow epistemic norms because 
truth and understanding (say) are worth having. But there is a dilemma for objective value-centered meta-epistemology.
The values grounding the norms are either solely product-focused or they include process-dependent ones. 

Product-focused values are things like truth and perhaps understanding, which are valuably possessed
regardless of how one came to have them. Imagine that truth is the value that grounds epistemic norms.
Presumably, the value of truth is weighted by the importance of the subject matter, so let us consider
a list of the most important questions there are. Perhaps near the top of the list will be questions
like whether there is objective morality, whether God exists, what the fundamental laws of physics
are, and so on. Suppose a trustworthy alien now offers to brainwash you so that you will find yourself
with correct beliefs as to the answers to the top fifty questions weighted by importance, so deeply 
implanted in you that you won't be able to revise your belief on these matters. The price is that you
will forget that you took the offer, and hence you will have no idea where these beliefs come from.
If truth is the grounding value of epistemic norms, it is difficult to either
fault your acquisition of the answers or to negatively epistemically evaluate the resulting beliefs. Yet 
these beliefs will by and large be unjustified.\footnote{Granted, sometimes not only a first-order question $Q_i$ 
will be found among the top fifty questions but so may be the second-order question $Q_j$ of what the evidence for
the answer to $Q_i$ is. But for the really important questions, plausibly the first-order question is
quite a bit more important than the second-order one, and hence typically the justification questions won't
make it on the top-fifty list.} You will not be in a healthy epistemic state. 

On the other hand, process-dependent values will be things like justification, where the way in which
the state has been achieved is essential to its being valuable. But as subsequent sections of this
chapter will show, what processes are appropriate for us as humans involves many seemingly arbitrary
parameters and is likely to be species-relative. This lets the Aristotelian answer Mersenne
questions raised by the process-dependent values: why should we live our epistemic lives by means
of \textit{these} processes with \textit{these} parameters?

Additionally, objective value-based answers fail to explain the agent-centeredness of epistemic excellence, 
which is if anything even more evident than in the moral case. A popular Youtuber who has imparted 
mathematical truth and understanding to millions of viewers is not an excellent epistemic agent if 
their own intellect (perhaps outside of mathematics) is filled with conspiracy theories and irrational 
beliefs. Yet the number of instances of true beliefs produced by the mathematical videos might vastly 
outnumber the number of false beliefs held by the one presenter. This is true even if we include 
process-dependent values: the Youtuber could in principle themselves think badly while successfully teaching 
people how to think well. 

We could, of course, simply restrict the values in the value-based meta-epistemology to be
the values embodied in the agent's \textit{own} intellectual life. But this seems \textit{ad hoc}.
Moreover, it is an excessive reaction. For our epistemic norms include social ones. We are community-based
investigators. Imparting and receiving things of epistemic value seems to be a central part of our
epistemic life. The Aristotelian can acknowledge that virtuous intellectual exchange is a part 
of the nature of our intellectual functioning, while nonetheless insisting that our primary epistemic
responsibility is to act well \textit{ourselves}, much as in the ethical case.

Virtue-based approaches to meta-epistemology are perhaps not radically different from
process-dependent objective value-based ones. After all, it is plausible that the answer to the question of why I should
investigate virtuously is that a process of virtuous investigation is valuable, and conversely
that a valuable process of investigation is precisely one that is virtuous. However, the
virtue-based approaches have a better answer to the agent-centeredness concern: it is natural
to think that there is something particularly compelling about being virtuous \textit{oneself}.
Still, virtue-based  approaches also need to answer Mersenne questions, just as virtue-based
appraoches did in the ethical case. And, again,
what the epistemic virtues are is likely to be species-relative. Or so I will argue. It may well
be that epistemic virtue does ground epistemic normativity, but human nature grounds what counts as
epistemic virtue and why epistemic virtue is something we should strive for.
...
how well previous inductive generalizations have fared, and even on pragmatic factors. But that added complexity does
nothing to make the Mersenne question easier to answer. And, as we discussed in Section~\ref{ch:ethics}.\ref{sec:two-step-vagueness}, invoking vagueness does not
solve the problem, and if anything multiplies the complexity even further.
...
\section{Logics of induction}\label{sec:Carnap}
Attempts have been made to give precise answers to questions about the reasonableness of inductive 
inferences using a rigorously formulated logics of induction, most famously by \citeauthor{CarnapLFP2} (\citeyear{CarnapLFP2}, \citeyear{CarnapContinuum}). Let us suppose, first, that some such
...
parameters controlling the speed of inductive inference) and those without. 
...
But \posscite{GoodmanFFF} famous riddle notes that all the emeralds we've observed are also grue, but it's not reasonable infer 
that all emeralds are grue. Let's say an emerald is grue if it is observed before the year 2100 and green, or if it is 
blue and not observed before 2100. According to Goodman (with year modified), the predicate ``is green'' is \textit{projectible}, i.e., amenable to 
...
whose symbols are terms for perfectly natural concepts, ones corresponding to fundamental joints in reality~\parencite{LewisNewWork},
...
relational or absolute. As \citeasnounwithpages{GoodmanFFF}{p.~106} himself observed, what we 
have instead is a range of predicates that are more or less projectible. We have ``is green'' and
...
\section{Epistemic value}\label{sec:epistemic-value}
\subsection{Epistemic value on its own}
\subsubsection{Scoring rules}\label{sec:scoring-rules}
...
$p$ in $H$ if $H$ is in fact false. The pair $T$ and $F$ is called an accuracy scoring rule.\footnote{The literature
tends to focus more on inaccuracy scoring rules, which are cost or penalty functions~\parencite{PreddEtAl}. 
An inaccuracy scoring rule is the negative of an accuracy scoring rule and \textit{vice versa}, so one can freely
go back and further between the two options. Since plausibly the good is more fundamental than the bad, it makes sense
to take accuracy scoring rules as more fundamental, however.}
...
Propriety mutually constrains the choice of $T$ and $F$. Given any non-decreasing function $T$ which is finite at
$1$, there is a
...
any non-increasing function $F$, there is a $T$ unique up to an additive constant such that $T$ and $F$ is a proper scoring rule (see Appendix~A).
...
\citeasnoun{GreavesWallace} have proved that if the scoring rule is strictly proper, for each experiment that has potential relevance 
...
will indeed be important, with different strictly proper scoring rules yielding different 
decisions (see Section~\ref{sec:pathological} and Appendix~B to this chapter for an example).
...
scoring rule, in the light of information-theoretic and other arguments~\cite{McCutcheonLog}. In our setting where we are
...
\subsubsection{Two approaches}\label{sec:pathological}
...
Let $T_0(x)=1000$ if $x\ge 0.999$, $T_0(x)=-999000$ if $x \le 0.001$, and $T_0(x)=0$ otherwise. Let $F_0(x)=T_0(1-x)$.
Then the pair $T_0$ and $F_0$ is a symmetric and proper scoring rule (see Appendix~B).
...
chance that the verdict is erroneous. Since $F_0(0.0009)=T_0(0.9991)=1000$ and $T_0(0.0009)=F_0(0.9991)=-999000$, it follows 
that the expected epistemic value, conditionally on winning the lottery, will be:
$$
    \frac{0.9991 \cdot 1000+0.0009\cdot (-999000)}2+\frac{0.9991 \cdot 1000+0.0009\cdot (-999000)}2 = 100 > 0.
$$
...
However, we will show in the Appendix of this chapter that there is a sequence of continuous, strictly proper, finite and symmetric
...
Epistemic normativity governs not merely doxastic states like beliefs and credence, but there are also what
\citeasnoun{FriedmanZetetic} has called ``zetetic norms'' about the \textit{pursuit} of truth (or other epistemic
values)---about what investigations
to undertake and so on. And here we have interaction between epistemic values and other values.
...
\section{Bayesianism}\label{sec:Bayesianism}
...
comparisons between probabilities~\parencite{KLST}, interval-valued probabilities where a range of numerical probabilities is instead of a specific numerical
probability~\parencite{Kyburg1961,Whitcomb2005}, and accounts on which conditional probabilities
are fundamental~\parencite{HajekCond}. Most of the issues raised will apply 
...
logic~\parencite{CarnapLFP2}, for instance, had a division
...
Intuitively, while Carnapian $c^*$ probabilities do support induction, they make induction too slow---it is only when we have observed the bulk of the cases
...
\citeasnoun{CarnapContinuum} himself eventually explored continua of methods of induction which varied in speed of induction.
...
Finally, it is worth noticing the interesting way in which this Aristotelian account of original priors makes 
justification both internalist and externalist. It is like an internalist account in that what justifies having 
an original prior is internal facts about the agent: the norms grounded in the agent's form, which form
constitutes the agent as a human being and hence as an agent. But like in externalist justifications, these normative
internal facts are not directly available to the agent's consciousness. They can be teased out by generous application of
Aristotelian optimistic methodology to our intuitions and to the patterns of inference exhibited by the movements 
between evidence and posteriors among those agents whom our intuitions hold up as exemplars of rationality
(as in the maxim that as for priors, ``by their posteriors ye shall know them''). Again, as in the ethical 
case (Section~\ref{ch:meta}.\ref{sec:ethics-internalism}),
this lets us have \textit{something} of internalism, while yet preserving some of the advantages of externalism.

\subsection{Anti-skepticism}\label{sec:skepticism}
...
There are many well-known problems with this approach. Some of these are technical. A classic
example is Bertrand's Paradox: What is the probability that a random chord drawn in a circle will
be shorter than the side of an equilateral triangle inscribed in the circle? There is more than one
way of applying the the principel of indifference, yielding different results~\parencite{Shackel2007}.
Or consider the Hausdorff Paradox: Given the Axiom of Choice, if $S'$ is the surface of a sphere $S$ with a
certain countably infinite set removed, then there are three disjoint subsets $A$, $B$ and $C$ whose
union is $S$ such that the four sets $A$, $B$, $C$ and $B\cup C$ (the union of $B$ and $C$) are all 
congruent---i.e., each of the four sets can be rotated around the center of the sphere to give any of
the others~\parencite{Hausdorff}. Since $A$, $B$ and $C$ are congruent, Indifference tells us 
that $z$ is equally likely to be in $A$ as in $B$ and $C$. It follows that $z$ is twice as likely 
to be in $B\cup C$ as in $A$.  But likewise $A$ and $B\cup C$ are congruent, so
Indifference tells us that $z$ is equally likely to be in $A$ as in $B\cup C$, a contradiction.\footnote{$^*$If
$P(U)$ is the probability that $z$ is in $U$, then $2P(A)=P(A)+P(A)=P(B)+P(C)=P(B\cup C)$ by the first Indifference argumentand since $B$ and $C$ are disjoint, while $P(A)=P(B\cup C)$ by the second argument. The only way both
equations can be true is if $P(A)=2P(A)$, i.e., $P(A)=0$. But then $P(S')=P(A\cup B\cup C)=3P(A)=0$. However,
since $S'$ omits countably infinitely many points of $S$, and the probability of a random point landing in a 
countably infinite set is zero or infinitesimal, $P(S')$ is either one or one minus an infinitesimal, so we
cannot have $P(A)=0$.}

But there are also two non-technical problems. First, in many cases it is
...
Second, if we are not very careful, we will completely undercut induction.
...
canonical answer. As we discussed in Section~\ref{sec:Carnap}, Carnap thought that lumping together sequences 
...
We can call these priors $L$-Solomonoff priors~(\citeauthor{PrussRegularityForward}, \citeyear{PrussRegularityForward};
cf.\ \citeauthor{Solomonoff78}, \citeyear{Solomonoff78}).
...
is much less likely to be randomly generated than \dref{all-black}, and it is intuitively very likely (though proving this rigorously would likely be quite difficult) that in general the probability of generating a sentence representing 64 black pawns is higher than that of 
...
the 64th pawn being black on the first 63 being black to be very high. (Maybe even too high? It is very difficult to 
do anything other than use intuition, because there are so many ways that a single situation can be 
...
The standard way to defend algorithmic measures of complexity from the problems presented by a plurality of languages
...
lottery which is known to have many paradoxes~\parencite[Chapter~4]{PrussInfinity}. A similar difficulty arises if we first choose a positive integer
...
Recently, \citeasnoun{ClimenhagaBasic} introduced the 
very helpful notion 
...
supported as a basic belief required of all human beings by their nature~\parencitecf{PlantingaBasic}.
...
\subsection{Non-Bayesian update}\label{sec:change-priors}
...
This suggests that we will need to quantify epistemic value, much as we did in Section~\ref{sec:scoring-rules}, and then say
...
Bayesian update any differently. For as we noted, \citeasnoun{GreavesWallace} proved that in the kinds of cases where one simply receives
evidence and sets one's credences according to it, conditionalization is the unique best policy for maximizing expected
epistemic value as measured by a strictly proper scoring rule.\footnote{Cf.~\citeasnoun{IsaacsRussellUpdating} 
discuss how maximizing such epistemic value also helps in cases where you are unreliable in fulfilling your
epistemic plans.}

However, the choice of a scoring rule or measure of epistemic value involves infinitely
many free parameters (cf.\ Section~\ref{sec:scoring-rules} and Lemma~\ref{lem:T2F} in Appendix~A), and at the same time not every scoring rule that satisfies the formal constraints is rationally
...
similar to the kinds of messy everyday ethics cases that are grist for the situationist's mill (cf.\ Chapter~\ref{ch:ethics}).
...
\ditem{update-1}{if you learn that $p_1$ is false, do Bayesian update}
\ditem{update-2}{if you learn that $p_1$ is true, change your credences to specific numbers $t_1,...,t_n$, respectively.}
...
As already noted more than once, your expected epistemic utility, defined using a proper scoring rule and calculated by your 
own lights, will be maximized by Bayesian update over the space of all update strategies~\parencite{GreavesWallace}. But the crucial thing to 
...
argue in the affirmative in Chapter~\ref{ch:God}). However, evolution might be metaphorically said to have ``access'' to more 
...
that we can do better than our instinctive ways of reasoning\footnote{Say, those of \posscite{Kahneman} System~1.}, 
especially given large amounts of scientific data,
say by adopting Bayesianism as our update rule. For the argument I want 
to press, I am willing to concede the point. But it is not clear that the same would have been true at earlier points in our species' intellectual development. And it \textit{is} clear that the same is not true for all possible species 
...
\section{The knowledge-first alternative}
Suppose that knowledge is a highly natural informationally complex universal with very complex necessary 
truths associated with it, much like obligation and love are suggested to be in Platonic approaches 
in Section~\ref{ch:ethics}.\ref{sec:complex-universals}. Some of the Mersenne questions above can be given answers in terms of knowledge, on this
knowledge-first account~\parencitecf{WilliamsonKn}. Thus,
maybe it is reasonable to believe that all ravens are black when one has observed enough ravens to gain
knowledge.\footnote{Knowledge-first as an alternative to the Aristotelian account was suggested to 
me by Charity Anderson, as was this solution to the raven problem.} Likewise, perhaps, 
the right priors are ones such that relying on them leads to knowledge when everything else goes
right (though one might be reasonably suspicious whether one can make this condition precise).

Essentially the same objections, however, can be given to the knowledge-first solution here as to the
ethical Platonist approach in terms of informationally complex universals (cf.\ Section~\ref{ch:ethics}.\ref{sec:complex-universals}). We do not
have an adequate knowledge-first solution to the problem of the appearance of contingency in the norms.
Moreover, there are a number of epistemological phenomena that are unlikely to be account for with the
binary relation of knowledge between an agent and a proposition. For instance, it is typically better to 
have a justified higher credence in a truth, even if this does not change whether one has knowledge,
for instance because one already had enough evidence for knowledge, and there will be apparently arbitrary
parameters in the mapping from credence to value, perhaps of the sort discussed in the context of scoring
rules (Section~\ref{sec:scoring-rules}, above).
It does not seem likely that such a function from credence to value can be accounted for in terms of 
knowledge.
...
Recently, \citeasnoun{JeffreyGood} has argued that ethical norms may be relative to a stage in life. Whether this is so, it is very
...
involve any pre-training. Our discussion of skepticism in Section~\ref{sec:skepticism} implies that it is normal for us to have certain substantive 
...
snake pictures~\parencite{BertelsSnakes}. But what those priors should be is surely highly 
...
cells with the fetus's genotype are found in women's healthy tissue after 
pregnancy~\parencite{ODonoghueMicrochimerism} as well as in microchimerism in twins~\parencite{WaszakTwins}. 
While we do not at present know of any benefit to the organism from such mechanisms\footnote{On the contrary,
fetal microchimerism may have unfortunate autoimmune consequences~\parencite{ODonoghueMicrochimerism}.}, we can imagine cases where
there are benefits, say by one organism giving cells with a new genotype to an organism whose genotype is either
defective (as we do artificially in somatic cell therapy\footnote{This has for instance been done in animal models
for colorblindness, with potential for future human treatment~\parencite{HassallColorblindness}.} or less well
...
theory. There is thus \textit{some} acceptable minor paternalism---we need to prudent in deciding what to say. And, of course, some paternalism, whether epistemic or moral, is acceptable with respect to one's children.\footnote{For
a helpful survey on epistemic paternalism, see \citeasnoun{JacksonPaternalism}.}
...
for utilitarianism~\parencite[p.~388]{ParfitRP}. On an additive account, letting a sufficiently large number of people know some trivial
...
In \citeasnoun{PrussInfinity} I argued that a good solution to epistemological paradoxes like this is to reject the metaphysical possibility
...
situations, just as in Section~\ref{ch:meta}.\ref{sec:outlandish1} it was suggested that our ethical norms do not apply to far-fetched situations. Whether this is 
...
In Section~\ref{ch:meta}.\ref{sec:superero}, we saw that a Natural Law ethic can easily accommodate the concept of supererogation by allowing for a case
...
Does this phenomenon appear in epistemology as well? Intuitively, it does. 
There are two varieties of this phenomenon. One kind, discussed by \citeasnoun{HedbergSuper}, 
involves deliberate actions directed at epistemic ends that go over and beyond one's epistemic
duties. These actions may involve overt physical activity such as pulling out a credit card
to check an already-memorized PIN number or reading encyclopedia articles, but also internal 
mental activity such as deliberate reflection on one's beliefs. 

A second kind involves epistemically skillful ability of a sort that may not be voluntary,
such as the evaluation of evidence or the formation of beliefs. Often, when Dr.~Watson fails to 
see the connections that Sherlock Holmes sees, we cannot say that Watson is epistemically 
defective---rather, Holmes is, at times, superlatively insightful in weighing the evidence. 
The distinction between insufficient insight, acceptable insight and superb insight is indeed 
familiar to us.

An Aristotelian metaepistemology neatly accommodates both forms of supererogation as part of the 
general normative phenomenon of functioning better than the norm requires, with the relevant aspect
(in the present case, the epistemic one) of human teleology explaining what counts as better and 
what counts as worse. The possibility of 
epistemic supererogation thus provides some support for the present account.

If the correct epistemology is Bayesian one might initially think there is no room for supererogation
of the non-voluntary sort. Either one updates
correctly on the evidence or one updates incorrectly. There is, no doubt, a gradation among failures of correct update, 
but there is only way to update correctly.  Thus, it seems, on a Bayesian epistemology, there is no room for supererogation of the non-voluntary sort,
and an Aristotelian's metaepistemologist's ability to account for epistemic supererogation is less of an advantage,
especially if Hedberg's voluntary supererogatory acts can be accounted for in terms of a non-epistemic normativity,
such as pragmatic or moral.

But there are at least three areas within a reasonable Bayesianism where there is room for supererogation even of the non-voluntary sort.
...
or indoors.) But I ignore it. And that's part of being an acceptable human reasoner. Similarly, when Dr.~Watson ignored the dog's
famous failure to bark in his holistic evaluation of the evidence~\parencite[p.~111]{Doyle1927}, he wasn't irrational. Sherlock Holmes, on the other hand, in focusing
...
Thus, even a Bayesian should be glad for the possibility of accounting for epistemic supererogation, and
...
question for epistemic rationality has received comparatively little attention.
...
Indeed, there are proposed norms which can be justified in this way. As noted a number of times, if one measures the utility of a 
doxastic state by a strictly proper scoring rule, then by the agent's own lights Bayesian update is 
an algorithm that maximizes epistemic utility. Similarly, it seems reasonable to think that the
epistemic rationality involved in choosing which evidence to pursue, i.e., which experiments to perform, 
can be accounted for by such a maximization. And finally the norm that one's credences should be 
(probabilistically) consistent can be justified by score maximization, because it is a theorem
that if one's credences are not consistent, there is a set of credences that are guaranteed to yield 
a better score no matter what the truth turns out to be, given some mild continuity conditions on the
strictly proper scoring rule.\footnote{\citeasnoun{PreddEtAl} prove this for additive rules. \citeasnoun{PettigrewNew}
announced a result for continuous rules, which was subsequently proved by \citeasnoun{NielsenDom} and
\citeasnoun{PrussDom}, and a precise  characterization of exactly how much continuity is needed is now
known~\parencite{PrussCont}.}

 If Bayesian update, choice of experiments and consistency
...
But this hope may not be borne out. As argued in Section~\ref{ch:ethics}.\ref{sec:risk}, there is good reason to think that prudential 
...
But more seriously, as already seen in Section~\ref{sec:Bayesianism}, Bayesian update, consistency and the norms governing choice of experiment do not exhaust the 
scope of epistemic norms, especially in the context of requiring substantive
...
There is also another possible answer to the ``Why bother?'' question. It could be that to the extent
...
way discussed in Chapter~\ref{ch:meta}. But in any case we should not take the moral account of why we should be 
...
and the will as a system directing behavior in the light of practical good-directed reasons (Section~\ref{ch:meta}.\ref{sec:moral-functionalism}). We might try now for
...
had by an agent (though in cases where one changes one's priors reasons may be involved---cf.\ Section~\ref{sec:change-priors}). Again, even if objective 
...
flourishing of the will, which facts in turn are grounded in our form. Analogically, we would expect epistemologically 
...
\textit{studiositas}~\parencite[II-II, 166--167]{AquinasST}, and the distinction lies in \textit{curiositas}'s trivial pursuit of mere bits of knowledge while the virtue pursues a holistic
...
\section{Appendix A: $^*$Generating scoring rules}
The following lemma is closely related to the \citeasnoun{Schervish} integral representation, but does not
appear to be in the literature.

\begin{lem}\label{lem:T2F}
Let $T$ be any monotonic non-decreasing function on $[0,1]$ that is finite except perhaps at $0$. 
Let
$$
	F(x) = -\frac{xT(x)}{1-x}+\int_{1/2}^x \frac{T(u)}{(1-u)^2} \, du,
$$
for $x<1$. If $T$ is continuous at $1$, let $F(1)=\lim_{x\to 1-} F(x)$, and otherwise
let $F(1)=-\infty$. Then the limit is defined if $T$ is continuous at $1$, and in any case the pair $(T,F)$ is proper.
\end{lem}

\begin{proof}
First consider the case where $T(x)$ is an indicator function of $A\subseteq [0,1]$, i.e., the function 
 $1_A(x)$ which is $1$ if
$x\in A$ and $0$ if $x\notin A$. For monotonicity, we must have $A=[a,1]$ or $A=(a,1]$. Let:
$$
...
and let $G_A(1)=-\infty$. Otherwise, $F_A$ is continuous at $1$, and let
$$
    G_A(1)=-\frac{a}{1-a}\cdot 1_A(1),
$$
with the convention that this is everywhere $0$ if $A=\varnothing$. It is easy to check that the pair
$(1_A,G_A)$ is proper.

Now let $T_A = -1_A$ where $A$ is of the form $[0,a]$ and $[0,a)$. Let:
$$
	F_A(x) = -\frac{xT_A(x)}{1-x}+\int_{1/2}^x \frac{T_A(u)}{(1-u)^2} \, du
$$
for $x<1$, and $F_A(1)=-\infty$ if $A=[0,1)$ while otherwise $F_A(1)=0$.
Note that $T_A$ differs by a constant from $1_{A^c}$ and $F_A$ by a constant from $G_{A^c}$. Thus
$(T_A,F_A)$ is also proper.

Now given any non-decreasing $T(x)$ bounded above (which follows from its being non-decreasing and finite
at $1$), without loss of generality assume $T$ is non-positive
(subtract a constant if necessary). Let $A_t=\{ x : T(x) \le t \}$. Then $A_t$ is of the form 
$[0,a]$ or $[0,a)$ and:
\begin{equation}\label{eq:Tint}
	T(x) = \int_{-\infty}^0 T_{A_t}(x) \, dt.
\end{equation}
Let $F$ be as in the statement of the Lemma. Then for $x<1$ we have:
\begin{equation}\label{eq:Fint}
\begin{aligned}
	F(x) &= -\frac{xT(x)}{1-x}+\int_{1/2}^x \frac{T(u)}{(1-u)^2} \, du \\
		 &= -\frac{x}{1-x}\int_{-\infty}^0 T_{A_t}(x) \, dt + \int_{1/2}^x \int_{-\infty}^0 \frac{T_{A_t}(x)}{(1-u)^2} \, dt\, du \\
		 &= \int_{-\infty}^0 \left(-\frac{xT_{A_t}(x)}{1-x}  + \int_{1/2}^x \frac{T_{A_t}(x)}{(1-u)^2} \, du \right) \, dt\\
		 &= \int_{-\infty}^0 F_{A_t}(x) \, dt,
\end{aligned}		 
\end{equation}
where the order of integration was swapped by Tonelli's theorem as $T_{A_t}\le 0$. By propriety of
$(T_{A_t},F_{A_t})$ we have:
$$
	p T_{A_t}(p) + (1-p) F_{A_t}(p) \ge 
	p T_{A_t}(x) + (1-p) F_{A_t}(x),
$$
for $p,x<1$, 
...
Note that $F_{A_t}$ differs by a constant from $G_{A_t^c}$ on $[0,1)$, and hence is non-increasing on $[0,1)$.
By \eqref{eq:Fint} this is also true of $F$, so it has a limit at $1$. Define $F_A$ to be that limit if
...
In Section~\ref{sec:pathological}, I claimed that this rule was proper and that it can be written as a limit of
...
Given a function $T$ satisfying the conditions of Lemma~\ref{lem:T2F}, write $T^\dag$ for the function $F$
specified by Lemma~\ref{lem:T2F}. It is straightforward to verify\footnote{I verified ths with Mathematica software.} that $F_0=T_0^\dag$, and hence by the Lemma, the pair $(T_0,F_0)$ is proper.

All we now need to show is that is that we can approximate $(T_0,F_0)$ with symmetric, finite, continuous and proper scoring rules.
...
Let $T_n$ be the (unique) continuous piecewise-linear function that (a)~is linear on each of the closed intervals $[0,0.001]$, $[0.001,0.001+1/n]$, $[0.001+1/n,0.999-1/n]$, $[0.999-1/n,0.999]$ and $[0.999,1]$, and (b)~agrees with
$T_0$ on $[0,0.001]$, $[0.001+1/n,0.999-1/n]$ and $[0.999,1]$. This function is monotone non-increasing, bounded 
in absolute value by $999000$, and converges pointwise to $T$ everywhere on $[0,1]$ as $n\to\infty$.

For $n>0$, let $F_n=T_n^\dag$ so that $(T_n,F_n)$ is proper. Note 
that $F_n$ is continuous as $T_n$ is. We also have $F_0=T_0^\dag$.
The bounded convergence theorem then shows that 
\begin{equation}\label{eq:limFn}
\lim_{n\to\infty} F_n(x) = F_0(x)
\end{equation}
for $x\le 0.999$. But since $T_n$ agrees with $T_0$ for $x>0.999$, \eqref{eq:limFn} is true for all $x$.
\chaptertail 
