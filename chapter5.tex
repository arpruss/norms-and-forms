...
probability that the $64$th square will be black is still $1/2$. On the other hand, if quantifiers and identity are allowed into our
language, then the all-black-pawn situation can be briefly represented by
$$
    \forall x(\operatorname{BlackPawn}(x))
$$
(where the domain is
...
$$
    \forall x(\Not (x=64)\rightarrow \operatorname{BlackPawn}(x))\And \operatorname{WhitePawn}(x).
$$
Since the probability of generating a given sequence of symbols decreases exponentially with the number of symbols, the second
sentence is much less likely to be randomly generated than the first, and it is intuitively very likely (though proving this rigorously would
...
estimates here, because there are many ways that a given situation can be represented.)
    
??Variations in ending method
