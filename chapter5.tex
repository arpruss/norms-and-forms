...
\citeasnoun{GreavesAndWallace} have proved that if the scoring rule is strictly proper, for each experiment that has potential relevance 
...
are fundamental~\parencite{Hajek2003}. Most of the issues raised will apply 
...
Bayesian update any differently. For as we noted, \citeasnoun{GreavesAndWallace} proved that in the kinds of cases where one simply receives
...
Does this phenomenon appear in epistemology as well? ??ref:https://philpapers.org/archive/HEDESA.pdf  Intuitively, it does. Often, when Dr.~Watson fails to see the 
...
This would be convenient to my argument, but is not quite correct. There are at least three areas within a reasonable Bayesianism where there is room for supererogation.
