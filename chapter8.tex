...
The most prominent Humean account of laws is \posscite{LewisCounterfactuals} Best System Analysis (BSA). The non-probabilistic version says that laws are the 
...
in terms of counterfactuals, and counterfactuals in terms of similarity of worlds, with similarity of laws prioritized~\parencite{LewisArrow1979,LewisCounterfactuals}.
The resulting account has pleasing ideological parsimony: both laws and causation reduce to the arrangement of matter,
an arrangement called ``the mosaic''.
...
problem independent of BSA. The old Deductive Nomological (DN) model of explanation in the philosophy of science\footnote{For a historical discussion, see \citeasnoun{SalmonFourDecades}.} had it
...
deductive schema, and cases where the explanandum is not even probable given the explanans are particularly 
problematic, with the classic case being a patient's having untreated syphilis explaining why the patient got paresis,
even though paresis is not a typical outcome of untreated syphilis~\parencite[p.~49]{SalmonFourDecades}. It was also seen that having this form is not sufficient for explanation, as one needs somehow to account
...
the shadow~\parencite[p.~47]{SalmonFourDecades}.
...
explanation of \dref{DN1}~\parencite{LoewerLawsTime}. Nonetheless, it is not clear that it matters for the anti-loop intuition that the two directions of an 
...
For suppose you light a match in the kitchen. Then the friction applied to the match explains why something is burning in 
...
causal, nomic or grounding. And an explanatory loop with respect to generic explanation is problematic.

Or consider the following plausible transitivity principle:
\ditem{wlange}{If $D$ partially grounds $E$, and $E$ partially scientifically explains 
$F$, then $D$ partially explains $F$.\footnote{\citeasnoun{LangeGrounding} gives a version of this principle with the
stronger conclusion that $D$ partially \textit{scientifically} explains $F$. This stronger conclusion may not be 
justified, however. For the partial ground of a scientific explanandum could itself be something outside the domain
of science. For instance, many theists will hold that the existence of any creature is partly grounded in God's
activity---indeed, people talk of God as ``the Ground of Being''. But now consider the scientific explanation of 
birds in terms of the prior existence of dinosaurs from which they evolved. On the theistic story, the prior existence
of dinosaurs is partly grounded in God's activity, and it is reasonable to conclude that therefore the existence of
birds is partly explained by God's activity---but not that it is partly \textit{scientifically} so explained.
Even apart from theism, there is likely a layer of fundamental philosophical ontology---say, objects having 
tropes---that grounds the explanans in a typical scientific explanation, and yet it seems wrong to invoke the 
objects of philosophical ontology in scientific explanation. (Interestingly, our weaker version of the 
transitivity principle does not suffice for the anti-Humean argument for which Lange designed his principle,
unless one additionally posits the transitivity of partial grounding.)}} But now the sun's gravitational attraction partly
grounds \dref{DN1}, which partly scientifically explains the attraction on the DN model, making the sun's
gravitational attraction be partly self-explanatory by \dref{wlange}.

We have thus seen that paradigmatic cases of DN explanations can be troubling even without Humean assumptions. However, there is a plausible explanation for the initial intuitive 
...
universe is low\footnote{See \citeasnoun{ChenPH}, who also argues that the past hypothesis can be seen as a fundamental
law on some non-Humean accounts.}, because the past hypothesis is so informative when combined with other laws, even though it is a hypothesis that applies
to only one time. Second, on standard Lewisian BSA, the laws are the logical consequences or theorems of the best system. But a universal 
...
has been disputed.\footnote{As a Christian, Peter van Inwagen presumably believes that miracles, such as the
resurrection of Jesus, have happened. But he defines miracles as events inconsistent with the conjunction of
the past and laws~\cite{PvIMiracles}. Indeed, in conversation, Peter van Inwagen has admitted that a false universal generalization can still be a law, if the exceptions are 
miracles.} We need not settle this question. If the argument is not deductively valid, it is still a good ampliative argument
...
While adding irrelevant facts to an argument's premises does nothing to damage the argument, irrelevance
damages an explanation~\cite{SalmonStat}. But on BSA, the proposition that by law all massive objects attract gravitationally tells us
...
\subsection{Magic}\label{sec:magic}
...
enter explanations \textit{as laws}, notwithstanding the argument of Section~\ref{sec:law-explanation}. Still, the vagueness of the concept of law would affect what counts as an explanation, assuming that generalizations that
...
Furthermore, our now familiar Aristotelian solution to issues of unacceptable vagueness, which is to ground boundaries in human nature, is not plausible in the case of laws of nature, because doing so would render laws of nature 
too anthropocentric. 
...
historical Hume's scepticism about induction) that we can thus reason \textit{deductively} from present pain to facts about contingent future arrangements
...
\subsubsection{Violation of the Principal Principle}\label{sec:principal-principle}
...
\posscite{LewisSubjectivist} Principal Principle (PP) holds that there is a tight epistemic relationship between chances and epistemic 
...
also be one's epistemic probability.\footnote{There are some technical refinements needed to handle the problem of
undermining futures~\cite{HallUndermining,LewisUndermining,ThauUndermining}, but the example below does not involve undermining futures.} 
I will now an example that shows a tension between PP and PBSA.

Imagine a world where all the coins have always lain scattered on a large flat surface. There is an 
...
the gold coin is heads or tails, though you know that it is one or the other, and that there are no further coins.
...
% Now, the following version of the Principal Principle is very plausible:
...
\subsubsection{$^*$A betrayal of frequentist intuitions}
In defining the laws, we are maximizing a balance of fit and brevity. 
Again, suppose a world with such a vast number of coins that their probabilistic distribution will
is sufficiently informative that it will be included in the laws. If half of the coin tosses are heads, 
and coin toss results are sufficiently important for the universe's behavior, then a law $U$ assigning 
chance $1/2$ to heads is likely the best balance. 

But now suppose that the frequency of heads isn't $1/2$
...
Let's suppose the number of coins $N$ is very large but easily expressible (e.g., $N=2^{2^{2^{2^{2^2}}}}=2^{2^{65536}}$), and let $\scr W_N$ be 
...
Thus, on PBSA, it appears that $U$'s being a law logically rules out some frequencies closer to $1/2$ while 
being compatible with some frequencies further from $1/2$. This seems to be a betrayal of the frequentist
intuitions behind PBSA. 
...
being zero and truth being one)---perhaps a measure according to some standard scoring rule (cf.\ Section~\ref{ch:epistemology}.\ref{sec:scoring-rules}).
...
there couldn't be a world where $T$ is supplemented by additional laws, since $T$ would entail that only the 
provable consequences of $T$ are laws.}
...
mosaic, and is harder to refute by counterexample. The problem that coincidental correlations become 
magical laws (cf.\ Section~\ref{sec:magic}) is less pressing once it is clear that the magical laws are not causal laws---for causation is 
...
Now let us recall the example from Section~\ref{sec:law-explanation}:
...
That said, there is a complication. Some Aristotelians will agree with \citeasnoun{vanInwagenMaterialBeings} that
the only substances are living things and simples, and hence there are no stars (this will also be an upshot
of the arguments of cf.\ Section~\ref{ch:metaphysics}.\ref{sec:living}). If so, we will need to modify the story, e.g., to talk about the plurality of
substances that are Sunwise arranged:
...
substances do not have substances as parts~\cite[VII.16]{AristotleMet}.\footnote{\citeasnoun{Devereaux} has argued,
however, that earlier in his career, Aristotle did allow substances to be parts of substances.} But then if a rabbit is a substance, the electrons in
...
Why do electrons in oaks, rabbits and humans behave the same way? To give a partial answer, we might posit a ``fine structure'' in the forms, whereby the oak, 
...
Another example is Einstein's Principle of Relativity, that all laws of nature hold in all reference frames.\footnote{\citeasnoun{LangeMeta} argues that such meta-laws are also a problem for Humean best-systems accounts.}

A second kind of problematic law is exemplified by the Second Law of Thermodynamics. The Second Law of Thermodynamics
has long presented a puzzle given that the underlying physics relevant to the behaviors of the particles whose joint entropy tends to increase
is invariant under a time-reversal. The usual contemporary solution is that what explains the increase of entropy is a physical dynamics
...
entropy, as it were, has nearly nowhere to go but up---there are a lot more possibilities for transitions from low to high entropy states
than from low to low entropy states. Thus the Second Law is not grounded in the natures of the interacting objects, 
...
This may seem to involve us in all the difficulties of BSA. But that need not be the case. First, we \textit{might} only make the move for
non-stochastic laws, denying that structural or impure generalizations that are stochastic in nature (such as, alas, the Second Law of 
...
explanatoriness of the laws here does not flow from BSA. For instance, on this story we might happily embrace the plurality of accounts of
bestness, because the laws explain simply in virtue of their content, not in virtue of their being \textit{laws}
or members of the best system.
...
\citeasnoun{ArmstrongWorld} grounded laws of nature in nomic necessitation relationships between universals. For instance, 
...
anyway. However, this
difference can be bridged: we can suppose a variant of the Armstrong view where nomic necessitation
is strengthened to broadly logical
entailment.
...
complete past states necessitate complete future states and \textit{vice versa}. While we may not live in such 
a world, nonetheless such a world appears metaphysically possible.

\subsection{Pushy global laws}
An account on which there are irreducible pushy global laws---fundamental aspects of reality that move stuff
around---also has many of the same advantages over Humeanism that Aristotelianism
does. But it is worth noting that there may be some advantages of an Aristotelian account.

First, causal powers are familiar things: we have them ourselves, and exercise them whenever we move. Global pushy laws of nature are rather more mysterious. 
Metaphysically contingent principles moving stuff around are puzzling. The idea of
\textit{things} moving things is very familiar, but \textit{principles} moving things are more mysterious, though granted
they do have roots going back to pre-Socratic \textit{archai}.

One difficulty with pushy global laws is that we should take seriously
the hypothesis that we inhabit a multiverse with different universes having significantly different patterns of physical behavior. 
For if the pushy global laws are truly global, they will exist at the level of the multiverse as a whole, which forces the
laws of physics to be complex, generating different patterns of physical behavior in different universes. They might do this by there being different types of particles or fields in different universes, and the global laws imposing 
different patterns of behavior on 
different types of things, or by their directly imposing different patterns of behavior on different regions of the multiverse. 

The Aristotelian, on the other hand, can simply ground the differences of behavior between universes by supposing 
the substances in different universes to have different forms. There is no need to \textit{further} suppose 
complex conditional laws that impose different behavior on different kinds or regions: the kinds themselves
are responsible for the differences in behavior	. This is perhaps a neater solution. 
...
be mid-sized substances like ourselves that are parts of the universe-sized substances. 
