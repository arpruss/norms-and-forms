...
Now imagine a universe consisting of a single quantum ``coin toss'' performed infinitely many times. Observing some 
infinite sequences of these coin tosses, such as $HHH...$, $TTT...$, $HTHT...$ or $THTH...$, would make us confident 
that the coin tosses are deterministic. But even though this would make us confident of determinism (e.g., $HTHT...$
...
guarantees} a deterministic law. This is highly counterintuitive. Furthermore, suppose that we in fact have a 
``patternless'' sequence that to the Humean yields indeterministic law that says that each coin toss is independent and
fair. But while the statement that each coin toss is independent and fair should allow any infinite sequence of coin
tosses, it turns out that certain sequences, such as our four examples above, are logically incompatible with 
independence and fairness on our Humeanism.\footnote{$^*$If independence and fairness is understood purely as mathematicians
understand it, this is somewhat defensible. ???....finish}

 certain infinite
sequences---say, the four given above---are incompatible with this law.

%I begin by claiming that on some plausible assumptions about our universe and its analogue nature, on the best
%Humean views you absurdly have a power to determine, in exhaustive detail albeit perhaps non-intentionally, events in far-away 
%galaxies by waving a magic wand. 
%
%???explain best Lewisian views
%
%Now on to our magic wand. Suppose you have a stick, which by waving you can position at any precise angle from vertical.
%Perhaps you can't \textit{intentionally} make the angle be exactly $45^\circ$ from vertical---complete
%precision is beyond us---but you can do so non-intentionally: you can aim to point the stick around $44^\circ$
%and happen to get exactly $45^\circ$. The cardinality
%of the set of deviation angles is equal to the number of real numbers between $0$ and $90$, which is the cardinality
%of the continuum. 
%
%Next, let's divide spacetime into two parts, where $C$ is the closed light-cone centered on the stick-waving event,
%i.e., the set of points that can be reached by traveling at
%or below the speed of light starting with your stick-waving event, and $U$ is the rest. Relativity
%Theory strongly suggests that what happens within $C$ does not affect what happens in $U$.\footnote{Quantum 
%entanglement may complicate the matter. If so, assume that there is no relevant entanglement here.} But now
%note that the set of physically possible states of a universe relevantly like ours at points in $U$ also has 
%the cardinality of the continuum. To see this, first consider a toy model where the universe consists of a 
...
%at any point in $U$ by specifying them at a countably infinite number of points of time (a continuous function on the 
%real line is fully described by specifying the values at all the rational numbered points of the real line, and the
%infinity of these points is countable). In other words, the universe within $U$ can be fully described by 
...
%of points $(a,b,c)$ from $A$, $B$ and $C$ respectively is also countable).??ref While this is a classical toy model, 
%it is worth noting that typical quantum mechanical models involve a separable Hilbert space of states, which has 
%continuum many states.\footnote{??continuity of evolution?}
%
%Thus, it is possible to find a function $f$ from stick angles to states of the part of the universe in $U$, such that 
%any  possible such state is equal to $f(x)$ for some stick angle $x$. Furthermore, it is possible to make $f$ be a 
%relatively simple and easily described function.\footnote{$^*$It's just a matter of encoding a countable sequence $r_1,r_2,...$ of
%real numbers into a single real number, and this can be done in an elegant way. We can remap any real numbers to 
%be in the range from zero to one, exclusive, using the function $g(x) = (1 + (x / (1+|x|)))/2$, and so we can
%suppose all the real numbers are between zero and one, exclusive. Then we can then form a real number by writing
%zero, a decimal point, then the first digit after the decimal point of $r_1$, then the second digit after the decimal
%point of $r_1$, then the first two digits after the decimal point of $r_2$, then the third digits after the decimal
%point of $r_1$ and $r_2$ respectively, and the first three digits after the decimal point of $r_3$, then the fourth 
%digits after the decimal point of $r_1$, $r_2$ and $r_3$ respectively, and the first four digits after the decimal
%point of $r_4$, and so on. The result is a single real number that neatly encodes the whole sequence of real numbers.}
%
%Now, let $x$ be the precise angle such that $f(x)$ encodes the state of the part of the universe in $U$. 
%Now, because events in the light-cone $C$ do not affect ones in $U$, if you were to position the stick at angle
%$x$, which is within your physical abilities, the events in $U$ would be just as they are, and hence the the stick's 
%angle would perfectly predict the state of the universe in $U$. Given that $U$ is intuitively likely to be a much larger portion
%of the universe than $C$---the universe likely extends far beyond $C$---the regularity between the position of the stick
%adn the rest of the universe 
