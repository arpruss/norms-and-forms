...
What is surprising, however, is that likely  there are some members $\alpha$ and $\beta$ in $F_N$ such that $\alpha<\beta<1/2$
...
There is a way out of this difficulty, but it involves dropping the idea that laws are propositions. And there is some independent reason
to do this. For if the law that the chance of heads is $1/2$ expresses a proposition, we would expect this to be a proposition that is
logically compatible with the hypothesis that the frequency of heads is $0$. But on PBSA, if the frequence of heads is $0$, there is no
way to get a chance of heads of $1/2$. 

Here is the suggestion. Suppose that we have a language $\scr L$ whose vocabulary can express all the concepts acceptable to the Humean,
plus which has one more function symbol, $\Chance(x)$ or $\Chance(x\mid y)$ (depending on whether we want fundamental chances to be 
