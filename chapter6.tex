...
On both paths, one will want to distinguish ``humans'' in \dref{human} and \dref{human-mammal} into 
human animals and human persons. Premise \dref{human} is true only of human persons, while \dref{human} is true
only of human animals. Thus the argument is unsound if ``humans'' is used consistently and otherwise invalid. 
And so on both Cartesianism and colocationism the intermediate step \dref{mammal} is denied. Instead, we are 
\textit{associated} with mammals, by ensoulment or by constitution. 

Yet \dref{mammal} is by itself extremely plausible. To deny that we are mammals seems akin to denying that earth
is round. One needs an extremely good argument for such denial. 

On the Cartesian side, there are well-known arguments in the philosophy of mind for the hypothesis that our minds are
immaterial.  But these arguments do not establish that \textit{we} are immaterial, absent an argument that, contrary
to ordinary talk of ``having a mind'', we \textit{are} minds. ???

??cerebra


Yet there are a number of philosophers who reject the conclusion that we are animals. 
