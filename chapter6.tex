...
Begin with the well-known observation that everyday causal systems, like the electrons buzzing inside a rock, can be re-interpreted as 
emulating the functioning of our brains.??ref If this is right, then functionalism appears to lead to the absurd thesis that rocks
not only think, but think like we do. A technical version for this argument will be given in ??forward:appendix.
...
Consider a simple Newtonian particle $P$ moving through space in a straight line. Now consider a determinisitic functional system $Q$ consisting of a 
finite number of possible subsystems $S_1,...,S_N$, where $S_k$ is always in exactly one state from the set $\scr A_k = \{A_{k,1},...,A_{k,n_k} \}$
at each of the discrete ``significant'' moments of time $t_1,t_2,...$.\footnote{A standard modern digital computer only has defined 
...
computationally defined. A lot of careful engineering goes into ensuring that the states become properly ``digital'' at the clock ticks.}
 We can think of $Q$ as a finite digital computer. The total state of the system at any given time can be represented as the
$N$-tuple $(a_1,...,a_N)$ where $A_k$ is a state in $\scr A_k$. Furthermore, we suppose there are functional laws which provide a 
mapping $f$ from the state of the system at time $t_i$ to the state at time $t_{i+1}$. Thus, $f(a_1,...,a_N)= (b_1,...,b_N)$ provided
that the system would transition from state $(a_1,...,a_N)$ to state $(b_1,...,b_N)$.\footnote{The mapping is independent of time. But we can
always suppose there is a finite clock, given by a subsystem $S_k$ such that the set of states $\scr A_k$ is
the set of times during the system's lifetime. Then the total system state at time $t_{i+1}$ can depend on the state of the clock subsystem,
which allows the system to handle time-dependent transitions.}
